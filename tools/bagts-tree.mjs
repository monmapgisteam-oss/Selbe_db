// БАГЦЫН мөрийн модыг үүсгэгч. Excel-ийн `*_final_publish` хуудасны шатлалт
...
// дэд мөрүүдээ томъёондоо нэрлэсэн байдаг — тэр нь эргэлзээгүй эх сурвалж.
//
 ...
 * дэд мөр гэж уншвал мод эвдэрнэ — тэдгээрийг ЗААВАЛ хасна.
 */
const BLD = new Set([...LAY.act, ...LAY.plan]);
const OK = (c) => c === "C" || c === "D" || c === "E" || c === "H" || BLD.has(c);

// Excel бүлгийг гурван янзаар нийлбэрлэдэг — аль нь ч ижил дэд мөрүүдийг нэрлэнэ.
function parseKids(rawF, rn) {
   ...
   * задлагч «H багана, мөр 11» гэж уншаад ХУДАЛ дэд мөр үүсгэдэг байв.
   */
  ...
  //    мөрийг өөрийнх нь дэд мөр болгож мэдэнэ.
  ...
  //    (нэг ганц дэд мөртэй бүлэг ч бодит байдаг).
  const byCol = new Map();
 ...
 * хуудаснаас шууд татдаг тул тэдгээрийн 8 ажил дээд мөргүй үлдэж, дээд түвшний мөр
 * мэт харагдана (бүлэг нь хоосон, ажлууд нь тархай).
 *
 * Дүрэм: № нь ҮСГЭЭР эхэлбэл БҮЛГИЙН толгой («А. Бэлтгэл ажил», «Б1»), түүний
 * дараах ТООН №-тэй дээд мөргүй мөрүүд түүнд харьяалагдана. Томъёогоор дээд мөр нь
 ...
 */
{
  ...
  if (fixed) console.log("№-ээр нөхсөн дээд мөр:", fixed);
}
...
// ⚠️ Гол шалгуур: «дээд мөр = өөрөөсөө бага гүнтэй хамгийн ойрын дээд мөр» гэсэн
...
if (process.argv[4] === "ts") {
  ...
  fs.writeFileSync(
    ...
    `// АВТОМАТААР ҮҮССЭН — ${process.argv[6] ?? base} мөрийн мод.\n` +
      ...
      `// Дээд мөр нь = өмнөх мөрүүдээс өөрөөсөө бага гүнтэй хамгийн ойрын мөр\n` +
