...
function FinCard({
   ...
   *
   * ⚠️ ШОШГЫГ ЯЛГАВ (2026-09-11). Урьд нь энэ нь «Нийт төсөв» гэж бичигддэг
   * байсан бөгөөд ДАШБОАРДЫН KPI-ийн «Нийт төсөв» ч мөн ижил нэртэй атлаа
   * ХОЁР ӨӨР ТОО гаргадаг байв:
   *
   ...
   * нь ялгана:
   *   · `planTotal` нь Cashflow-гийн АЖЛЫН мөр БҮРИЙН гэрээний дүнг
   *     (`Geree_erh_dun`, хоосон бол `Urdch_tusuwt_urtug`) багцын түлхүүрээр
   *     нийлүүлнэ. Тиймээс 5·6·7-р хэсгийн (дэд бүтэц) ажлууд ОРНО —
   *     Excel-ийн «НИЙТ» томьёонд тэдгээр ордоггүй.
   *   · Харин БАГЦЫН ТҮЛХҮҮРГҮЙ 13 мөр (5 нь «БАГЦ 1-4» мэт ДИАПАЗОН)
   *     `pkgKeyOf` -ээр `''` болдог тул энэ нийлбэрээс ХАСАГДАНА — амьдаар
   *     88.9 тэрбум ₮. (`pkgKeyOf`-ийн ⚠️: диапазоныг буруу багцад наахаас
   *     сэргийлэх нь дүнг алдагдуулахаас чухал.)
   * Өөрөөр хэлбэл энэ тоо = «БАГЦАД ХОЛБОГДСОН гэрээний дүн», дашбоардынх =
   * «төслийн батлагдсан нийт төсөв». Хоёр өөр хамрах хүрээ.
   *
   * ⚠️ Тиймээс энэ хоёрыг ХАРЬЦУУЛЖ «зөрүү нь алдаа» гэж БҮҮ хай — 2026-09-11
   * -нд хэмжиж баталсан. Зөрүү ӨӨРЧЛӨГДВӨЛ л шалга.
   */
  const note =
    total > 0 ? (
      <span className={ts.totNote}>
        {tr('Багцын гэрээний дүн:')} <b>{num(total)} ₮</b>
      </span>
    ) : undefined;
