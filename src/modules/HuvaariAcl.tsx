 ...
 * аккаунт нэмээд түүнд багц зүүдэг тул «Багц 3.1-ийг хэн хариуцаж байна» гэдгийг
 * харахын тулд бүх мөрийг гүйлгэж үзэх шаардлагатай байлаа. Ажил нь БАГЦААР
 * хуваарилагддаг тул дэлгэц ч мөн багцаар байх ёстой.
 *
 ...
 *
 * ⚠️ ХАДГАЛАЛТ нь ХЭРЭГЛЭГЧЭЭР (`huvaariAcl`: нэг аккаунт = нэг мөр). Энэ
 * панел нь түүнийг БАГЦААР эргүүлж харуулна — өөрөөр хэлбэл дэлгэцийн бүтэц
 * ба хадгалалтын бүтэц ӨӨР. Багцад аккаунт нэмэх нь тухайн хэрэглэгчийн
 * мөрөнд тэр багцыг нэмэх үйлдэл.
 */

import { useEffect, useState } from 'react';
import { t as tr } from '@/lib/i18nCore';
import {
  ALL_BAGTS, huvaariFailedUsers, listHuvaariAssigns, removeHuvaariAssign, setHuvaariAssign,
  subscribeHuvaariAcl, type HuvaariAssign, type PlanRole,
} from '@/lib/huvaariAcl';
...
const roleLabel = (r: PlanRole): string =>
  (r === 'author' ? tr('Зохиогч') : tr('Батлагч'));

const ROLES: PlanRole[] = ['author', 'approver'];

/** Тухайн багцад тэр үүргээр хуваарилагдсан аккаунтууд */
const usersOf = (rows: HuvaariAssign[], group: string, role: PlanRole): string[] =>
  rows
...
export function HuvaariAcl() {
  ...
  const rows = listHuvaariAssigns();
  const failed = new Set(huvaariFailedUsers());
  /*
   ...
   *    `{roles[], bagts[]}` буюу үүрэг × багцын ҮРЖВЭР (`huvaariScope` нь
   *    хоёрыг ТУСАД нь шалгадаг), харин панел нь багц бүрд, үүрэг бүрд
   *    тусдаа хөзөр харуулна. Тиймээс «Багц A · Зохиогч»-той хүнийг «Багц B ·
   *    Батлагч» болгож нэмбэл тэр нь Багц A-д ч БАТЛАГЧ, Багц B-д ч ЗОХИОГЧ
   *    болж, хүсээгүй эрх чимээгүй тарж, Багц A нь `stuck` (зохиогч=батлагч)
   *    болж ГАЦНА. Шинэ үүрэг ЭСВЭЛ шинэ багц нэмэх нь одоо байгаа нөгөө
   *    хэмжээсийг өргөтгөх тохиолдолд үйлдлийг ЗОГСООЖ админд ил хэлнэ —
   *    хадгалалтын схемийг үүрэг тус бүрийн багцаар салгах хүртэлх хамгаалалт.
   */
  const addTo = (group: string, role: PlanRole, user: string) => {
    ...
    const r = setHuvaariAssign(u, roles, bagts);
    setErr(r.ok ? '' : (r.error ?? ''));
    void r.sync;
  };
   ...
   *
   * ⚠️ ГУРВАН ТОХИОЛДОЛ:
   *   · «Бүх багц»-тай хүн — энэ багцаас ганцаарчлан хасах БОЛОМЖГҮЙ (хүрээ
   *     нь тодорхой багцын жагсаалт биш). Админд ил хэлнэ.
  ...
  const removeFrom = (group: string, role: PlanRole, user: string) => {
    ...
    if (otherPkgs.length === 0 && otherRoles.length === 0) {
      if (!window.confirm(tr('«{0}»-г хуваарийн хуваарилалтаас бүрэн хасах уу? «Хуваарь төлөвлөх» ба «Хуваарь батлах» эрх нь мөн буцаагдана.', user))) return;
      void removeHuvaariAssign(user).sync;
      return;
    }
    if (otherPkgs.length === 0) {
      /* Багц нь ганц — үүргийг нь л хасна, багц хэвээр */
      void setHuvaariAssign(user, otherRoles, cur.bagts).sync;
      return;
    }
    /*
     ...
     *    НӨГӨӨ үүрэг нь ч чимээгүй алга болно: админ зөвхөн зохиогчийг
     *    хасахыг хүссэн атлаа тэр багцын цорын ганц батлагчаа алдаж, багц
     *    бүхэлдээ гацдаг байв. Ил татгалзаж, бүтэн дахин томилохыг хэлнэ.
     */
    ...
    void setHuvaariAssign(user, cur.roles, otherPkgs).sync;
  };
...
/** НЭГ БАГЦЫН хөзөр — Зохиогч ба Батлагчийн жагсаалт */
function PkgCol({
  ...
  onAdd: (group: string, role: PlanRole, user: string) => void;
  onRemove: (group: string, role: PlanRole, user: string) => void;
}) {
  const authors = usersOf(rows, group, 'author');
  const approvers = usersOf(rows, group, 'approver');
  /**
   * ⚠️ ГАЦААНЫ АНХААРУУЛГА: зохиогч нь бий атлаа батлагч нь ЗӨВХӨН тэр өөрөө
   *    бол илгээсэн хуваарийг нь хэн ч батлах боломжгүй болно
   *    (`decidePlan` өөрийгөө батлахыг татгалздаг). Багц бүхэлдээ гацна.
   */
  ...
  return (
    <div className={s.aclCol}>
      ...
      {ROLES.map((role) => {
        const list = role === 'author' ? authors : approvers;
    ...
    <div className={s.aclRole}>
      ...
      {list.length === 0 && (
        <div className={s.aclEmpty}>
          {role === 'author' ? tr('Зохиогч томилоогүй') : tr('Батлагч томилоогүй')}
        </div>
      )}
