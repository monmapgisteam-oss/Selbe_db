...
export function Iot({ dim, setDim }: { dim: Dim; setDim: (d: Dim) => void }) {
  ...
  /*
   * ⚠️ `keepOn: [tick]` — 5 минут тутмын дахин таталтад ХУУЧИН заалтыг дэлгэц
   * дээр үлдээнэ (stale-while-revalidate). Урьд нь `tick` нь энгийн dep байсан
   * тул `useAsync` нь `LOADING` руу шилжиж, БҮХ карт/чарт «уншиж байна» болж
   * анивчдаг байв — `loadSensors`-ийн TTL нь мөн 5 минут учир ихэнх tick
   * сүлжээ ч хөнддөггүй атлаа самбар бүхэлдээ хоосордог байлаа.
   *
   * ⚠️ `range` нь жагсаалтад ОРООГҮЙ: хүрээ солиход хуучин заалт шинэ
   * хүрээнийх БИШ тул урьдын адил «Татаж байна…» гарна.
   */
  const q = useAsync(() => loadSensors(range), [tick, range], { keepOn: [tick] });
  /**
