...
/* (3) Нийтэлсэн ба «Устгах» хоёулаа АЛСЫН хуулбарыг цэвэрлэнэ — эс бөгөөс
       ...
       ⚠️ 2026-09-25-аас нийтлэх нь ҮРГЭЛЖ, «Устгах» (`dropDraft`) нь түлхүүртэй
       үед алсыг УСТГАХГҮЙ, харин хоосон + tombstone-той (`del`) ноорог бичнэ —
       бусад төхөөрөмжийн хуучин хуулбар тэр нүдийг сэргээхгүйн тулд.
       `clearRemoteDraft(pkg.key)` дуудлага ЯГ 4 хэвээр (tombstone-гүй салбарт). */
...
/* ── НООРОГИЙН ХУГАЦАА — ЛОКАЛ 3 ХОНОГ · АЛСЫН ХЯЗГААРГҮЙ (2026-09-08, хэрэглэгч) ──
   ...
   ⚠️ ГОЛ ГЭРЭЭ: алсын хуулбар ХУГАЦААГААР ХЭЗЭЭ Ч устахгүй. Сэргээх эффектийн
   зомби-цэвэрлэлт (`rem && !remote → clearRemoteDraft`) нь `parseDraft`-ийн
   `null`-д тулгуурладаг тул алсын задлалт хугацаа шалгавал ArcGIS-ээс УСТГАНА. */
{
  const codeOnly = SRC.split('\n')
    .filter((l) => !/^\s*[*]/.test(l) && !/^\s*\/[*]/.test(l) && !/^\s*\/\//.test(l))
    .join('\n');
  assert.ok(!/\bDRAFT_TTL_MS\b/.test(codeOnly), 'хуучин нэгдсэн TTL буцаж орсон');
  assert.ok(!/\bDRAFT_FRESH_START\b/.test(codeOnly), 'шинэ эхлэлийн шүүлт буцаж орсон');
  /* локал TTL = 3 хоног */
  const m = SRC.match(/const LOCAL_DRAFT_TTL_MS = ([0-9 *]+);/);
  assert.ok(m, 'LOCAL_DRAFT_TTL_MS тогтмол алга');
  assert.equal(Function('return ' + m[1])(), 3 * 24 * 3600 * 1000, 'локал ноорогийн хугацаа 3 хоног биш');
  /* parseDraft нь эх сурвалжаар ялгана; хугацааг ЗӨВХӨН local-д */
  const parse = between('const parseDraft = ', 'const readDraft = ');
  assert.ok(parse.includes("source: 'local' | 'remote'"), 'parseDraft эх сурвалжаар ялгадаггүй');
  assert.ok(parse.includes("source === 'local' && Date.now() - d.t > LOCAL_DRAFT_TTL_MS"), 'локал хугацааны шүүлт алга эсвэл алсад ч хамаарч байна');
  assert.ok(parse.includes('if (!d.t || !Array.isArray(d.cells)) return null;'), 'бүтцийн шалгалт хэвээр байх ёстой');
  /* дуудлагууд зөв эх сурвалжтай */
  assert.ok(SRC.includes("parseDraft(raw, 'local')"), 'readDraft локал гэж дуудахгүй байна');
  assert.ok(SRC.includes("parseDraft(rem.payload, 'remote')"), 'алсын задлалт remote гэж дуудахгүй байна — хугацаагаар устах эрсдэл');
  assert.ok(!/parseDraft\([^,)]*\)/.test(codeOnly), 'parseDraft эх сурвалжгүй дуудагдсан');
}
console.log('✅ ноорогийн хугацаа — локал 3 хоног · алсын хязгааргүй · зомби-цэвэрлэлт хугацаагаар асахгүй');
...
/* (4c) НООРОГИЙН ХУГАЦАА — 2026-09-08-наас ХЯЗГААРГҮЙ (дээрх «хэзээ ч
   устахгүй» хэсэг барина). 3→14 хоногийн шалгуур энд байсныг хасав. */
