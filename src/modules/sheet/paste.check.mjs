...
/* ── Сөрөг утга БИЧИГДЭХГҮЙ, «буруу» гэж тоологдоно ──
   ⚠️ 2026-09-03: урьд нь `Math.max(0, …)`-аар ЧИМЭЭГҮЙ 0 болгодог байв.
   «БУУРНА» баталгаажуулалт нь `stored != null` шаарддаг тул ХООСОН нүдэнд
   буулгасан сөрөг тоо ямар ч асуултгүйгээр «хэмжигдсэн 0» болж бичигдэнэ —
   `null ≠ 0` дүрмийн шууд зөрчил. Одоо `commit`-тэй ижил: няцаана. */
{
  const { hits, bad } = planPaste(parseGrid('-7'), [0], 1, 0, 0, () => true);
  assert.equal(hits.length, 0, 'сөрөг утга бичигдэх ёсгүй');
  assert.equal(bad, 1, 'сөрөг утга «буруу» гэж тоологдох ёстой');
  console.log('✅ сөрөг утга няцаагдав');
}
