...
export default function FillNew({ view }: { view?: SheetView } = {}) {
  ...
  /**
   * ИЛГЭЭГДСЭН боловч БАТЛАГДААГҮЙ нүднүүд — `${oid}:${блок}`.
   *
   * ⚠️ Хуудсан дээр ГУРВАН эх сурвалжийн тоо зэрэг харагдана: архивт
   * батлагдсан · илгээгдсэн (хянагдаж байгаа) · ноорог (илгээгээгүй).
   * Урьд нь эхний хоёр нь ЯГ АДИЛХАН харагддаг тул бөглөгч «энэ тоо
   * хянагчид очсон уу, эсвэл аль хэдийн батлагдсан уу» гэдгийг ялгах
   * ямар ч аргагүй байв.
   */
  ...
  useEffect(() => {
    ...
    loadSchema(pkg)
      .then(async (schema) => {
        ...
        /* ⚠️ ИЛГЭЭГДСЭН НҮДИЙГ ТЭМДЭГЛЭНЭ (2026-09-06, хэрэглэгч: «ноорог
           болон датанаас авсан тоо хоёрын ялгааг хэрхэн ялгах вэ»).
           Хуудсан дээр ГУРВАН эх сурвалжийн тоо зэрэг харагддаг:
             · архивт БАТЛАГДСАН   — энгийн
             · ИЛГЭЭГДСЭН, хянагдаж байгаа (overlay) — урьд нь батлагдсантай
               ЯГ АДИЛХАН харагддаг байв, ялгах ямар ч арга байгаагүй
             · НООРОГ, илгээгээгүй  — `dirty` (тод, ногоон хүрээ)
           `overlaySubmission` нь аль нүдийг давхарласнаа `cellKeys`-ээр
           буцаадаг байсан ч ХААНА Ч уншигддаггүй байсныг ашиглав. */
        setSentKeys(new Set(ov ? ov.cellKeys : []));
  ...
  return (
    <div className={st.wrap} ref={wrapRef}>
      <div className={st.toolbar}>
        ...
        {/* ⚠️ ТАЙЛБАР — гурван төлөвийн ялгааг ҮГЭЭР хэлнэ (2026-09-06).
            Өнгө ганцаараа мэдээлэл дамжуулах ёсгүй (төслийн дүрэм) бөгөөд
            бөглөгч «аль тоо нь миний ноорог, аль нь хянагчид очсон, аль нь
            батлагдсан бэ» гэдгээ ЭХНИЙ хараагаар мэдэх ёстой.
            Зөвхөн ХАМААРАЛТАЙ үед гарна — юу ч бөглөөгүй бол чимээ болно. */}
        {!locked && (dirtyCount > 0 || sentKeys.size > 0) && (
          <span className={st.hint}>
            {dirtyCount > 0 && (
              <span className={st.legDirty} title={tr('Ногоон хүрээтэй нүд — та зассан, хараахан ИЛГЭЭГЭЭГҮЙ. «Илгээх» дарж хянагчид хүргэнэ.')}>
                {tr('ногоон: илгээгээгүй ({0})', dirtyCount)}
              </span>
            )}
            {dirtyCount > 0 && sentKeys.size > 0 && ' · '}
            {sentKeys.size > 0 && (
              <span className={st.legSent} title={tr('Шар зураастай нүд — илгээгдсэн, хянагчийн шийдвэр хүлээж байна. Батлагдсаны дараа тэмдэг арилна.')}>
                {tr('шар: хянагдаж байна ({0})', sentKeys.size)}
              </span>
            )}
            {' · '}
            <span title={tr('Тэмдэггүй нүд — батлагдаж үндсэн бүртгэлд орсон тоо.')}>
              {tr('тэмдэггүй: батлагдсан')}
            </span>
          </span>
        )}
                    ...
                    {sc.bld.map((b, bi) => {
                      ...
                      return (
                        <td
                          ...
                          /* ⚠️ ГУРВАН ЭХ СУРВАЛЖ, ГУРВАН ТЭМДЭГ (2026-09-06):
                             · `dirty` — ноорог, ИЛГЭЭГЭЭГҮЙ (ногоон хүрээ)
                             · `sent`  — илгээгдсэн, ХЯНАГДАЖ байгаа (шар зураас)
                             · аль нь ч биш — архивт БАТЛАГДСАН тоо
                             `dirty` нь `sent`-ээс ДАВАМГАЙЛНА: хэрэглэгч тэр
                             нүдийг илгээснийхээ ДАРАА дахин зассан гэсэн үг. */
                          className={cls(
                            "num bld" +
                              (canVol ? (noPerf ? " view" : " editable") : " calc") +
                              (dirty ? " dirty" : sentKeys.has(key) ? " sent" : "") +
