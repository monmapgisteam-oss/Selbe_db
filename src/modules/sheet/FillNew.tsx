...
/**
 * НООРОГИЙН АМЬДРАХ ХУГАЦАА — ЭХ СУРВАЛЖААР ЯЛГААТАЙ (2026-09-08, хэрэглэгчийн
 * шууд заавар: «локалыг 3 хоног, ArcGIS-ийг хэзээ ч устахгүй болгоё»).
 *
 *   · ЛОКАЛ (`localStorage`) — 3 хоног. Хөтчийн сан нь хэдэн зуун KB-ийн
 *     ноорог хуримтлуулах газар биш; 3 хоногоос хуучин локал хуулбар устсан ч
 *     АЛСАД (ArcGIS) бүрэн хуулбар байгаа тул ажил алдагдахгүй — дараагийн
 *     нээлтэд алсаас сэргэнэ. Локал нь зөвхөн «шууд, сүлжээгүй» зам.
 *   · АЛСЫН (ArcGIS) — ХЯЗГААРГҮЙ. Гүйцэтгэгч нэг сар талбайд ажиллаад буцаж
 *     ирэхэд ноорог хэвээр байх ёстой. Устах хоёр зам л бий, хоёулаа
 *     хэрэглэгчийн ИЛ үйлдэл: «Илгээх» · «Ноорог устгах».
 *
 * ТҮҮХ: 3 хоног (хоёуланд) → 14 хоног (2026-09-06) → хязгааргүй (2026-09-08
 * өглөө) → локал 3 / алсын ∞ (2026-09-08). `DRAFT_FRESH_START` (2026-09-03-ны
 * нэг удаагийн цэвэрлэлт) хасагдсан — түүнээс өмнөх ноорог аль хэдийн хаягдсан.
 *
 * ⚠️ АЛСЫН ЗОМБИ-ЦЭВЭРЛЭЛТ (`rem && !remote → clearRemoteDraft`) нь алсын
 *    хуулбарыг ЗӨВХӨН бүтцээр эвдэрсэн (задарч чадахгүй) үед л устгана —
 *    хугацаагаар ХЭЗЭЭ Ч БИШ. `parseDraft(raw, 'remote')` хугацааг шалгадаггүй
 *    тул тэр зам алсын ноорогийг хугацаанаас болж устгах боломжгүй.
 */
const LOCAL_DRAFT_TTL_MS = 3 * 24 * 3600 * 1000;
/**
 * АЛСЫН ХУУЛБАРЫН ЦАГИЙН ТОГТМОЛУУД (2026-09-08, хэрэглэгч: «локал 0 сек,
 * ArcGIS 3 сек болго»). Локал хадгалалт нь угаас засвар бүрд ШУУД (0 сек) —
 * `saveDraftLS` хадгалах эффектийн дотор синхрон ажилладаг.
 *
 * · REMOTE_DEBOUNCE_MS — бичихээ зогсоод хэдэн мс-ийн дараа ArcGIS руу
 *   илгээх. Хоёр компьютер хооронд шилжихэд хамгийн муудаа 3 секундын ажил л
 *   хоцорно (unmount/таб хаахад шууд явдаг тул тэр ч ховор).
 * · REMOTE_CAP_MS — тасралтгүй бичиж байхад ч ЭНЭ хугацаа тутам заавал
 *   илгээнэ (debounce хэзээ ч дуусахгүй тохиолдлын хамгаалалт).
 * · REMOTE_RETRY_MS — илгээлт унавал дахин оролдох хугацаа.
 *
 * ⚠️ ГУРВУУЛАА 3 СЕК (хэрэглэгчийн шууд заавар, 2026-09-08: «arcgis бүгд
 *    3 сек»). Cap = debounce гэдэг нь «3 секунд тутам заавал» — тасралтгүй
 *    бөглөж байхад ч 3 сек тутам нэг хүсэлт; retry = 3 сек гэдэг нь сүлжээ
 *    тасарсан үед 3 сек тутам дахин оролдоно. Нэг хэрэглэгчийн 1 хүсэлт/3 сек
 *    нь ArcGIS-ийн хязгаарт (org түвшинд мянгаараа/мин) огт хүрэхгүй.
 */
 ...
 *
 * ⚠️ БҮТЦИЙН шалгуур нь ЛОКАЛ ба АЛСЫН хоёуланд ЯГ ижил (2026-09-03): алсын
 *    хуулбар нь org доторх хэн ч засаж болох хүснэгтээс ирдэг тул итгэл нь
 *    localStorage-аас ИЛҮҮ БАЙХ ЁСГҮЙ.
 * ⚠️ ХУГАЦААНЫ шалгуур нь эх сурвалжаар ЯЛГААТАЙ (2026-09-08): `'local'` —
 *    `LOCAL_DRAFT_TTL_MS`; `'remote'` — хязгааргүй. Тайлбарыг дээр үзнэ үү.
 */
const parseDraft = (raw: string, source: 'local' | 'remote'): Draft | null => {
  try {
    const d = JSON.parse(raw) as Draft;
    /* Бүтцийн шалгалт: агшингүй эсвэл нүдгүй бичлэг нь ноорог биш. */
    if (!d.t || !Array.isArray(d.cells)) return null;
    /* ⚠️ Хугацаа — ЗӨВХӨН локалд. Алсын хуулбарыг хугацаагаар хаявал сэргээх
       эффектийн зомби-цэвэрлэлт түүнийг ArcGIS-ээс УСТГАНА — хэзээ ч болохгүй. */
    if (source === 'local' && Date.now() - d.t > LOCAL_DRAFT_TTL_MS) return null;
...
/**
 * ЛОКАЛ ба АЛСЫН ноорогийг НИЙЛҮҮЛНЭ — нэгийг нь сонгохгүй (2026-09-08).
 *
 * ⚠️ ЯАГААД: урьд нь `remote.t > local.t`-ээр ШИНИЙГ нь л авдаг байв. Оффисын
 * компьютер дээр 30 нүд бөглөөд (алсад хуулагдсан) гэртээ өөр 5 нүд бөглөвөл
 * гэрийн ноорог шинэ тул оффисын 30 нүд БҮХЭЛДЭЭ хаягдана — ямар ч
 * анхааруулгагүй. «Ноорог заримдаа алга болдог» гэсэн гомдлын нэг шалтгаан.
 *
 * ДҮРЭМ: нүд бүрд (`${oid}:${b}` түлхүүрээр) ШИНЭ ноорогийн утга ялна, зөвхөн
 * нэг талд байгаа нүд ХЭВЭЭР үлдэнэ. `dates` ижил. `adds` нь сөрөг oid-оор —
 * ижил oid бол шинэ талынх. `asOf` нь шинэ талынх (нэг л утга). `rowKeys` нь
 * хоёр талын нэгдэл (oid → танигч, шинэ тал ялна) — зөөлтөд хоёр талын мөр
 * ХОЁУЛАА танигчтай байх ёстой.
 *
 * ⚠️ Нэг тал `null` бол нөгөөг ШУУД буцаана — нийлүүлэх зүйлгүй.
 * ⚠️ Хоёулаа `null` бол `null`.
 */
const mergeDrafts = (a: Draft | null, b: Draft | null): Draft | null => {
  ...
  /*
   * ⚠️ ИЛГЭЭСЭН МӨР (`sent`, 2026-09-24 — `Draft.sent`-ийн ⚠️): хоёр талын
   *    нэгдэл (агшин их нь ялна); `adds`-д байгаа, батлагдаагүй, нэмсэн агшин
   *    нь илгээснээс хожуу БИШ мөрийг `oid:*` нүд/огноо/эзэн/агшинтай нь
   *    хасна (`a:` tombstone-той ижил). 7 хоногоос хуучныг хаяна.
   */
  const sent = new Map<number, number>(older.sent ?? []);
  ...
  for (const [o, a] of sent) {
    ...
    /* (`in`-ээр нарийсгана — TS cast бичихгүй: draft.check 4d энэ функцийг JS болгож ажиллуулдаг) */
    const row = adds.get(o);
    if (!row || ('ajilOid' in row && row.ajilOid)) continue;
...
export default function FillNew({ view }: { view?: SheetView } = {}) {
   ...
   *
   * ⚠️ СЕШН ХООРОНД САНАГДАНА (2026-09-15-ны хэрэглээний аудит). Урьд нь
   *    `useState(false)` байсан тул өдөрт 3–4 багц бөглөдөг хүн энэ товчийг
   *    өдөр бүр, хуудас нээх бүрдээ дарж байв — `fillMode` нь аль хэдийн
   *    ингэж санагддаг, зөвхөн энэ хоцорсон байлаа. Бүтэн дэлгэц нь зөвхөн
   *    ХАРАГДАЦ тул эрхийн ач холбогдолгүй.
   */
  const [wide, setWide] = useState(() => {
    try { return localStorage.getItem("selbe-fill-wide") === "1"; } catch { return false; }
  });
  useEffect(() => {
    try { localStorage.setItem("selbe-fill-wide", wide ? "1" : "0"); } catch { /* хаалттай орчин */ }
  }, [wide]);
  ...
  useEffect(() => {
    ...
    /* ⚠️ НҮД эсвэл КАЛЕНДАР нээлттэй бол Escape нь ЗӨВХӨН түүнийг хаана
       (2026-09-24): урьд нь оролтын хандлагч хаагаад л энэ хандлагч мөн
       ажиллаж, бүтэн дэлгэцээс гарч байв. Ref нь render бүрд тольдогддог тул
       нэг дарах дээр `setEdit(null)` хараахан буугаагүй ч энд «нээлттэй». */
    const esc = (e: KeyboardEvent) => {
      if (e.key !== "Escape") return;
      if (editRef.current || pickRef.current) return;
      setWide(false);
    };
   ...
   */
  const [resumedOid, setResumedOid] = useState<number | null>(null);
...
/*
  ...
  const [remoteState, setRemoteState] = useState<
    null | { kind: 'ok'; at: number } | { kind: 'big' } | { kind: 'fail'; why: string }
  >(null);
  /**
