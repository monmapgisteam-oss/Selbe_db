...
import { useBagtsTable } from '@/modules/Dashboard';
...
export function Tailan() {
  ...
  const send = useCallback(async () => {
    ...
    setBusy(true);
    try {
      await emailViaEml(rows, date || new Date().toLocaleString('mn-MN'), extra);
    } catch (e) {
          ...
          {(rows) => (
            <Data q={ex} loading="Гүйцэтгэл, санхүү, газар, дэд бүтэц, ХАБЭА-гийн өгөгдөл нэгтгэж байна…">
              {(x) => {
                ...
                const budget = rows.reduce((a, b) => a + b.budget, 0);
                        ...
                        <tbody>
                          {sorted.map((b) => (
                            <tr key={b.key}>
                              ...
                              <td className={r.num}>{b.budget > 0 ? bn(b.budget) : '—'}</td>
