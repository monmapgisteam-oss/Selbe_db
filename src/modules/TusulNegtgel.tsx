...
import { loadNegtgelFull, negtgelTree, type NegtgelRow, type NegtgelCalc } from '@/lib/negtgel';
...
function readPrefs(): ColPrefs {
  try {
    ...
    if (j && Array.isArray(j.hidden) && j.w && typeof j.w === 'object') {
      ...
      return { hidden: j.hidden.filter((k) => COL.has(k) && k !== LOCKED), w: j.w, freeze, wrap };
    }
...
function Tree({ rows, calc }: { rows: NegtgelRow[]; calc: Map<number, NegtgelCalc> }) {
  ...
  const left: Partial<Record<ColKey, number>> = {};
  ...
  const sty = (k: ColKey) => ({
    width: W(k), minWidth: W(k), maxWidth: W(k), left: left[k],
  ...
  const startResize = (k: ColKey) => (e: RPointerEvent<HTMLSpanElement>) => {
    ...
    const move = (ev: PointerEvent) => {
      const w = Math.max(MIN_W, Math.round(w0 + ev.clientX - x0));
      setPrefs((pp) => ({ ...pp, w: { ...pp.w, [k]: w } }));
    };
    const up = () => {
  ...
  const head = useMemo(() => {
    ...
    const perf = (plan: number) => (plan ? (t.act / plan) * 100 : 0);
    return {
      ...t,
      perfG: perf(t.planG),
  ...
  /* «ТӨСЛИЙН НИЙТ ГҮЙЦЭТГЭЛ» мөрийн нүднүүд — эхний харагдах Д/Д · Багц ·
     Нэр баганыг нэг нүдэнд нэгтгэнэ */
  const lead = vis.filter((c) => LEAD.includes(c.key));
  const leadW = lead.reduce((a, c) => a + W(c.key), 0);
  const total: Partial<Record<ColKey, string>> = {
    ...
    act: fmtPct(head.act, 2),
    perfG: fmtPct(head.perfG, 2),
    perfGch: fmtPct(head.perfGch, 2),
    perfGu: fmtPct(head.perfGu, 2),
  };
  ...
  return (
    <div className={n.wrap}>
      ...
      {hMenu && (() => {
        ...
        return (
          <>
            <div className={f.thMenuVeil} onClick={close} />
            <div
              className={f.thMenu}
            ...
            >
              ...
              <div className={f.thMenuCols}>
                ...
                >
                  {keys.length > 1
                    ? `Freeze ${keys.length} columns`
                    : freeze === k ? 'Unfreeze' : 'Freeze up to here'}
                </button>
          ...
          <tbody>
            ...
            <tr className={n.stage}>
              {lead.length > 0 && (
                <td
                  colSpan={lead.length}
                  className={`${n.fz} ${lead.some((c) => c.key === edge) ? n.fzEdge : ''}`}
                  style={{ width: leadW, minWidth: leadW, maxWidth: leadW, left: left[lead[0].key] }}
                >
