...
function ScheduleDetail({ fin, prog, bagts, pkgProg }: {
  ...
  return (
    <>
      ...
      <Panel title={tr('Төлөвлөгөө vs бодит — сараар')} note={tr('багцын дундаж %')}>
      ...
      <Panel title={tr('Төлөвлөгөөний хоцрогдол — багцаар')} note={tr('төл. − бодит, %')}>
        <Data q={pkgProg} loading={tr('Татаж байна…')}>
          {(list) => {
      ...
      <Panel title={tr('Сүүлийн сарын өсөлт — багцаар')} note={tr('сүүлийн хоёр бүртгэлийн зөрүү')}>
        <Data q={pkgProg} loading={tr('Татаж байна…')}>
          {(list) => {
      ...
      {/* САНХҮҮЖИЛТИЙН ХУРИМТЛАЛ — дээрх «Санхүүжилт — сараар» нь САР ТУТМЫН
          олголтыг харуулдаг: тэр нь сарын хэлбэлзлийг сайн заадаг ч «гэрээний
          хэдэн хувийг олгосон бэ» гэсэн асуултад хариулдаггүй. Хуримтлагдсан
          муруй нь төлөвлөгөөт хуваарийн эсрэг байрлалыг шууд харуулна. */}
      ...
      <Panel title={tr('Хоцрогдлын өөрчлөлт — сараар')} note={tr('төл. − бодит, багцын дундаж %')}>
        <Data q={pkgProg} loading={tr('Татаж байна…')}>
          {(list) => {
      ...
      {/* ЭЗЛЭХҮҮНИЙ БИЕЛЭЛТ — гүйцэтгэлийн % нь ЖИНГЭЭР бодогдсон нэгтгэсэн тоо;
          ...
          утга. */}
      <Panel title={tr('Эзлэхүүний биелэлт — багцаар')} note={tr('бодит / төлөвлөгөөт эзлэхүүн')}>
        <Data q={pkgProg} loading={tr('Татаж байна…')}>
          {(list) => {
            const rows = latestPkgProgress(list)
      ...
      <Panel title={tr('Тайлагналын идэвх — сараар')} note={tr('бүртгэл оруулсан багц')}>
        <Data q={pkgProg} loading={tr('Татаж байна…')}>
          {(list) => {
  ...
  return (
    <>
      ...
      {/* НИЙГМИЙН БАГЦЫН ТӨСӨВ — эдгээр багцын төсөв 08 «Хөрөнгө оруулалт»-ын
          ...
          ⚠️ `budget.byPkg` нь `CF004` (дэд багцтай) задаргаа — 08-ын картуудтай
      ...
      {/* ТӨСӨВТ ЭЗЛЭХ ХУВЬ — «нийгмийн дэд бүтэц 116.6 тэрбум ₮» гэсэн дүн
          ...
          ⚠️ Суурь нь `budget.total` (CF006, урьдчилсан төсөвт өртөг) —
