...
export function OverlayControl({
   ...
   */
  /**
   * ⚠️ Модулийн эзэмшил дангаараа хангалтгүй: ЗЭЭЛДСЭН давхарга (өөр модулийн
   * эзэмшилд ч энэ модульд бүрэн эрхтэй харагддаг) нь `l.module !== module` атлаа
   * зурган дээр идэвхтэй байдаг. «Тойм» модуль бүхэлдээ зээлдсэн давхарга дээр
   * ажилладаг тул үүнийг тооцоогүй бол блокийн давхарга ил байхад жагсаалтад
   * унтраалттай мэт харагдана.
   */
  const subKeyOf = (l: OverlayLayer) => (l.id.includes(':') ? l.id.split(':')[1] : l.id);
  const isActive = (l: OverlayLayer) =>
    l.module === module || (sublayers?.includes(subKeyOf(l)) ?? false);
