...
/**
 * УДИРДЛАГЫН ҮЗҮҮЛЭЛТИЙН ТООЦОО — ЗӨВХӨН ТООЦОО, дүрслэлгүй.
 *
 * ⚠️ 2026-09-06: энэ файл `ExecKpi.tsx` байсан бөгөөд тооцоо ба ДҮРСЛЭЛ хоёрыг
 * зэрэг агуулдаг байв. Хэрэглэгч тэр дүрслэлийг («таван ангилалд эгнүүлсэн
 * 28 мөр») ойлгомжгүй гэж үзэн БҮРМӨСӨН хассан — үзүүлэлтүүд одоо
 * `HomeBoard.tsx`-д ҮЙЛ АЖИЛЛАГААНЫ СХЕМИЙН зангилаандаа суудаг.
 *
 * ⚠️ ТООЦООГ ХАМТ ХАЯГААГҮЙ гэдэг нь САНААТАЙ. Энд `buildSchem()`-д ОГТ
 * БАЙХГҮЙ дохионууд бий бөгөөд тэдгээр нь хамгийн чанга нь:
 *     · Обьёмын зөрүү (`variance`) — 56.8 тэрбум ₮, ЯАРАЛТАЙ
 *     · Нэгж талбарын давхцал (`overlap`)
 *     · Санхүүжилт ба гүйцэтгэлийн зөрүү (`finance`)
 *     · Ослын хохирол (`damage`) · Нормын биелэлт (`planning`)
 * Дүрслэлтэй нь хамт устгасан бол эдгээр чимээгүй алга болох байв.
 *
 * ⚠️ ХООСОН ФУНКЦ БИШ, HOOK: доторх `useAsync`/`useMemo` нь дуудагч
 * бүрэлдэхүүний амьдралын мөчлөгт наалддаг. Дуудлага нь ЗААВАЛ бүрэлдэхүүний
 * дээд түвшинд байх ёстой (нөхцөлт дуудвал React-ийн hook дүрэм зөрчигдөнө).
 */
...
import { HABEA, CASHFLOW_NEW, cfMonthAxis, pkgKeyOf, type ViewKey } from '@/lib/services';
...
import type { Metric } from '@/lib/kpiModel';
 ...
 *
 * ⚠️ 2026-09-06: АЧААЛЖ БАЙГАА мөр одоо `loading` — урьд нь `unknown` байсан
 * тул «Өгөгдөл алга» гэж шошголж, нүүр нээгдэх эхний секундүүдэд бараг бүх
 * мөр «алга» гэж уншигддаг байв (хэрэглэгчийн гомдол, дэлгэцийн зураг).
 *
...
const notReady = (
  ...
  return { label, value: '…', note: loadingNote, level: 'loading' };
};
...
export function useExecMetrics(): Metric[] {
  ...
  const funnel = useMemo<Metric>(() => {
    ...
    const steps = [
      ...
      /* ⚠️ 2026-09-06: «Шилжүүлсэн» шат ХАСАГДСАН — «ӨМНӨХ ШИЛЖҮҮЛСЭН» мөрийн
         төрөл нь `cashflow_0813`-т байсан бөгөөд тэр үйлчилгээ хаягдсан. */
    ].filter((x) => x.v > 0);
  ...
  const finance = useMemo<Metric>(() => {
     ...
     * ⚠️ Тэнхлэгийг өгөгдөлд БАЙГАА саруудаас угсарч БОЛОХГҮЙ: 2026-01-д
     *    ямар ч хэмжилт алга тул график нэг слот гулсаж, тэрнээс хойшхи бүх
     *    багана буруу сар дээр зогсоно.
     */
    const all = cfMonthAxis().map((label) => {
      let sum = 0;
      finQ.data.given.forEach((byMon) => { sum += byMon.get(label) ?? 0; });
      return sum;
    });
  ...
  const schedule = useMemo<Metric>(() => {
    ...
    /* ⚠️ `d.contracts` — `Cashflow_0904`-ийн 76 мөр, мөр бүр НЭГ гэрээ.
       (Хуучин `cashflow_0813` нь гэрээ×үе грейнтэй 209 мөртэй байсан тул
       мөрийн төрлөөр шүүх ЗААВАЛ байсан; одоо шаардлагагүй.) */
