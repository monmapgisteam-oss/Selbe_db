...
/* 10 — бодит зам: түүхий мөр (бүгд group:false) + хуудасны мөр → бүлэг ялгагдана */
{
  ...
  /* Нэгтгэлд: flat багц унасанд тоологдохгүй, харин issues/facts-д ил */
  const k = computeQaqc([joined, attachOrFlat(P2, raw, null)], []);
  assert.deepEqual(k.failedSources, []);
  assert.equal(k.value, '3');                                // 1 (мод) + 2 (хавтгай: бүлэг+навч)
  assert.deepEqual(k.facts, ['60.0% нийт 5 ажлаас', '2 ажил хагас бүрдсэн', '1 багц мод холбогдоогүй']);
  assert.equal(k.issues.length, 1);
  assert.equal(k.issues[0].tone, 'warn');
  assert.equal(k.issues[0].text, `${P2.label} — мод холбогдсонгүй, бүлгийн мөр тоологдов`);
  ...
  assert.deepEqual(k2.issues.map((i) => i.text), [
    ...
    `${P2.label} — мод холбогдсонгүй, бүлгийн мөр тоологдов`,
  ]);
