...
export function computeQaqc(loaded: readonly QaqcLoaded[], failed: readonly string[]): KpiResult {
  ...
  const total = pkgs.reduce((n, p) => n + p.total, 0);
  const empty = pkgs.reduce((n, p) => n + p.empty, 0);
  const partial = pkgs.reduce((n, p) => n + p.partial, 0);
  ...
  const issues: KpiIssue[] = [
    ...
    ...flat.map((p): KpiIssue => ({
      text: tr('{0} — мод холбогдсонгүй, бүлгийн мөр тоологдов', p.pkg.label),
      tone: 'warn',
    })),
  ];
