 ...
 * ⚠️ Хэт гүн (5 м+) шатаавал үер голдоо «шингээд» эргийн бүс огт норохгүй
 *    болно — үерийн загварын утга алдагдана.
 */
...
export async function simulateFlood(
  ...
  /**
   * ГОЛЫН ГОЛДРИЛ — гадаргад шатаах цагирагууд (`loadRiverRings` §тайлбар).
   * ⚠️ ТАТАГДААГҮЙ БОЛ ШИНЖИЛГЭЭ ЗОГСОХГҮЙ: гол нь загварыг САЙЖРУУЛДАГ
   * оролт болохоос амин чухал биш. Сүлжээний саатлын улмаас «үерийн
   * шинжилгээ огт хийгдэхгүй» болох нь хамаагүй дор.
   */
   ...
   * ⚠️ Барзгар байдал (Маннинг) ба сувгийн сүлжээнд ЗААВАЛ нэгдэнэ: бодит
   * голыг «тал газар» гэж тооцвол урсгал хэт удааширч, голдрилоор явахын
   * оронд далан давна.
   */
   ...
   * ⚠️ Полигоны ирмэг нь ГАЗАР ЗҮЙН хаалт БИШ, зүгээр л «энэ цаанаас
   * цааш бодохгүй» гэсэн зураас. Тиймээс тэнд ус ЗААВАЛ гарна
   * (`outCoef`-ийн тайлбарыг үз).
   */
   ...
   */
  /**
   * ── ГОЛЫН ГОЛДРИЛЫГ ГАДАРГАД ШАТААХ (stream burning, 2026-09-15) ──
   *
   * ⚠️ ЯАГААД ЗААВАЛ: тооцооны нүд нь 16.9 м (DSM-ийн 5.6 м-ийн 3×3-ийн
   * ДУНДАЖ), харин Сэлбэ гол 5–15 м өргөн. Дундажлахад голын ёроол хөндийн
   * түвшинд «арчигдаж», ГОЛДРИЛ гэж нам зам ҮЛДЭХГҮЙ болно. Тэгэхээр ус
   * голын хөндийгөөр биш, санамсаргүй хотгоруудаар салангид тархана —
  ...
  if (river) {
    ...
    for (let y = 0; y < N; y++) {
      ...
      for (let x = 0; x < N; x++) {
        const i = y * N + x;
        if (!inRings(river, e0.xmin + (x + 0.5) * cw, wy)) continue;
        riverCell[i] = 1;
        z[i] -= RIVER_BURN_M;
      }
    }
  ...
  /* ⚠️ БОДИТ ГОЛ нь хуримтлалын босго давсан эсэхээс ҮЛ ХАМААРАН суваг:
     зурсан талбай нь голын ДЭЭД хэсгийг л хамарвал хуримтлал бага гарч,
     жинхэнэ голдрил «тал газар» гэж ангилагдана (2026-09-15). */
    ...
    for (let y = 1; y < N - 1; y++) {
      for (let x = 1; x < N - 1; x++) {
        ...
        for (const dnb of nbo) {
          ...
          const sl = (z[k] - z[i]) / dx;
          /**
           * ⚠️ ХИЙМЭЛ ЗАХ ДЭЭР УС ЗААВАЛ ГАРНА (2026-09-15-ны засвар).
           *
           * Урьд нь налуу гадагшаа өгсөж байвал `continue` хийдэг байсан нь
           * тэр захыг ТУСГАДАГ ХАНА болгодог: зурсан полигоны зах уул өөд
           * харсан хэсэгт ус гарч ч чадахгүй, доорх `outsideIdx` цэвэрлэгээнд
           * ч хүрэхгүй тул полигон дотроо хуримтлагдана — хэрэглэгчийн
           * «ус зурсан талбай дотроо тогтоно» гэсэн шүүмж яг энэ.
           *
           * Полигоны ирмэг нь газар зүйн саад БИШ (цаана нь жинхэнэ газар
           * бий, зүгээр л бодогдоогүй) тул ЧӨЛӨӨТ ГАРЦ — хамгийн багадаа
           * `S_MIN` налуугаар.
           *
           * ⚠️ ТОРНЫ жинхэнэ зах (`gridEdge`) дээр хуучин дүрэм ХЭВЭЭР:
           * тэнд уулын энгэрийн дээд зах байж болох бөгөөд гаргавал домэйн
           * руу орох ёстой борооны усыг сорж хаяна.
           */
  ...
  {
    /**
     * ⚠️ 2026-09-15 (дөрөв дэх засвар) — ДЭЭД УРСГАЛЫГ ӨНДРӨӨР тодорхойлно,
     * ЗҮГЭЭР БИШ.
     *
     * ХУУЧИН ДҮРЭМ БУРУУ БАЙВ: `if (y > N * 0.6) continue;` нь «гол хойноос
     * урагш урсдаг» гэсэн ЗҮГИЙН таамаглал байсан. Сэлбэ сав газрын хажуугийн
     * жалгууд нурууны энгэрээс гол руу ХӨНДЛӨН (зүүн, баруун тийш) урсдаг тул
     * тэр таамаглал тэнд ЭСРЭГЭЭРЭЭ ажиллана: оролт нь жалгын АДАГТ бууж,
     * 389 м³/с ус доороос дээш шахагдан «уул өөдөө урсаж» харагдана
     * (хэрэглэгчийн шүүмж, 2026-09-15 — скриншот).
     *
     * ШИНЭ ДҮРЭМ — зүг ОГТ хамаагүй: ус ӨНДРӨӨС нам руу урсдаг тул оролт нь
     * ирмэгийн ӨНДӨР талд, гаралт нь нам талд байна. Ирмэгийн нүднүүдийн
     * өндрийн МЕДИАНААС дээшхийг нэр дэвшигч болгож, дотроос нь хамгийн их
     * хуримтлалтайг (= голдрил хаана ирмэгийг огтолж байна) сонгоно.
     *
     * ⚠️ Дундаж БИШ МЕДИАН: нэг өндөр оргил дунджийг татаж, бүх нэр дэвшигчийг
     * таслах эрсдэлтэй.
     */
    const edgeCells: { i: number; a: number; z: number }[] = [];
    for (let y = 2; y < N - 2; y++) {
      for (let x = 2; x < N - 2; x++) {
        const i = y * N + x;
        ...
        if (!edge) continue;
        edgeCells.push({ i, a: accCells[i], z: z[i] });
      }
    }
    const zs = edgeCells.map((c) => c.z).sort((m, n2) => m - n2);
    const zGate = zs.length ? zs[(zs.length * 0.5) | 0] : -Infinity;
    let cand = edgeCells.filter((c) => c.z >= zGate);
    /* ⚠️ БУЦААХ ЗАМ: хаалга бүх нэр дэвшигчийг таславал (маш нарийн эсвэл
       бүрэн тэгш талбай) өндрийн шүүлтгүйгээр хамгийн их хуримтлалтайг авна —
       оролтгүй үлдэхээс хамаагүй дээр. */
    if (!cand.length) cand = edgeCells;
    ...
    /* ⚠️ `cand.length`-ээс ХЭТРЭХГҮЙ (2026-09-15-ны аудит ба `tailan` салбар
       ХОЁУЛАА ижил алдааг илрүүлсэн). Урьд нь `Math.max(3, …)` нь гурваас
       цөөн нэр дэвшигчтэй үед хязгаараас гарч `cand[k].i` дээр TypeError
       шидэж бүх симуляц унадаг байв — хэрэглэгч торны өмнөд хэсэгт (мөр
       695-ын шүүлт бүгдийг хаядаг) талбай зурахад бодитоор тохиолдоно.
       Нэр дэвшигч огт байхгүй бол оролт нэмэхгүй: голын урсац 0 болох нь
       тэр мужид ЗӨВ (зөвхөн бороогоор бодогдоно). */
    const take = Math.min(cand.length, 8);
  ...
  while (t < totalS && step < MAX_STEPS) {
    ...
    /* ── Голын оролт — ГИДРОГРАФААР (өсөлт → оргил → татралт) ── */
    {
      ...
      /* ⚠️ ОРОЛТГҮЙ ҮЕД АЛГАСНА: `inletArea = 0` бол `add = Infinity` болж
         бүх гүн NaN болно — дэлгэц дээр ус ОГТ харагдахгүй болж «загвар
         ажиллахгүй байна» гэж уншигдана (2026-09-15). Бороо нь хэвээр. */
      if (inlet.length) {
