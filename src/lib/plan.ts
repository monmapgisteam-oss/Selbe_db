...
export type PlanRow = {
  ...
  act?: (number | null)[];
  /**
   * БОДИТ эхэлсэн / дууссан огноо — блок бүрд, ms epoch (2026-09-23).
   *
   * ⚠️ `Span` БИШ, хоёр тусдаа массив: эхэлсэн ч дуусаагүй ажил (`aStart`
   *    бий, `aEnd` null) хамгийн түгээмэл төлөв — `Span` нь хоёулаа байхыг
   *    шаарддаг тул хагас бүртгэл `null` болж алга болно.
   * ⚠️ ЗӨВХӨН БҮРТГЭЛ: `propagate`/`rollUpGroups`/`statusOf` эдгээрийг
   *    ХАРДАГГҮЙ — төлөвлөгөө бол шийдвэр, бодит бол баримт.
   * ⚠️ Сонголттой (`?`) — `plan.check.mjs` зэрэг хиймэл мөрүүд байхгүйгээр
   *    үүсдэг; `Huvaari.toPlanRows` үргэлж өгнө.
   */
  aStart?: (number | null)[];
  aEnd?: (number | null)[];
  /** Хүн хүч · машин механизм — мөрийн нөөц (2026-09-23). `null` = бөглөөгүй. */
  hun?: number | null;
  mashin?: number | null;
};
