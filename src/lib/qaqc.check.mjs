...
/* 2 — зураглагдсан багц бүрийн хаяг ЗӨВ, хүснэгт нь давхардаагүй */
/**
 * ⚠️ БҮХ БАГЦ ХҮСНЭГТТЭЙ БАЙХ ШААРДЛАГАГҮЙ (2026-09-16). Нэмэлт багцууд
 *    (5.x · 6.x · 10) нь БАРИЛГА БИШ тул тэдэнд `Inspection Test Plan`
 *    үүсээгүй. `qaqcTableOf` нь `null` буцааж, «Чанар» хуудас тэр багцыг
 *    зүгээр л харуулахгүй — fail-closed, алдаа шидэхгүй.
 *
 * ⚠️ Гэхдээ БАРИЛГЫН багц бүр хүснэгттэй байх ЁСТОЙ: тэдгээрт чанарын акт
 *    хөтлөгддөг тул зураглал дутвал бөглөх газаргүй үлдэнэ. Барилгын багцыг
 *    `floors != null`-ээр ялгана (блоктой, давхраар салдаг).
 */
let mapped = 0;
for (const p of PKGS) {
  const ref = qaqcTableOf(p.key);
  if (p.floors != null) {
    assert.ok(ref, `${p.key} нь БАРИЛГЫН багц атлаа QAQC хүснэгт зураглагдаагүй`);
  }
  if (!ref) continue;
  mapped += 1;
...
assert.equal(mapped, 10, `QAQC зураглал 10 байх ёстой, олдсон: ${mapped}`);
...
/* (в2) АЖИЛ зөрвөл — № таарсан ч холбогдохгүй.
   ⚠️ Ажлын нэр нь одоо ЦОРЫН ГАНЦ баталгаа (2026-09-07) тул түүний хатуу
      чанарыг тусад нь бэхэлнэ. */
assert.equal(
  attachTree(three, [
    sr('A.', 'Бэлтгэл ажил', 0, true), sr('1', 'ӨӨР АЖИЛ', 1, false), sr('2', 'Гэрэлтүүлэг', 1, false),
  ]),
  null, 'ажил зөрсөн ч мод холбогдов',
);

/* (в3) № зөрсөн ч АЖИЛ таарвал ХОЛБОГДОНО — эх өгөгдлийн алдааг тойрно.
   ⚠️ QAQC хүснэгтийн `F_` нь найдваргүй: excel «3.10»-г «3.1» болгосон, мөн
      зарим бүлгийн дугаар хоёр хүснэгтэд жинхэнээсээ зөрдөг («ГДДС» 7 ↔ 4).
      Урьд нь № -г тулгадаг байсан тул шатлал 10/10 багцад ХЭЗЭЭ Ч
      холбогдож чадаагүй. 2026-09-07-нд 9 багцын 13,032 мөрд ажлын нэр
      100.00% таарсныг хэмжсэн — мөр зөв эгнэдэг нь батлагдсан. */
const noDiff = attachTree(three, [
  sr('A.', 'Бэлтгэл ажил', 0, true),
  sr('3.10', 'Хашаа', 1, false),        // QAQC-д «3.1» гэж хадгалагдсан хэлбэр
  sr('7', 'Гэрэлтүүлэг', 1, false),     // бүлгийн дугаар жинхэнээсээ зөрсөн
]);
assert.ok(noDiff, '№ зөрсөн ч ажил таарвал холбогдох ёстой');
assert.deepEqual(noDiff.map((r) => r.depth), [0, 1, 1]);
assert.deepEqual(noDiff.map((r) => r.no), ['A.', '1', '2'], 'QAQC-ийн ӨӨРИЙН № хэвээр үлдэнэ');
