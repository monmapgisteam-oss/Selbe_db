...
/* ── IPC-ийн хоёр дахь S-муруй (2026-09-10) ── */
{
  ...
  /*
   * Цэнхэр муруй = (cashflow хуримтлал − орон сууцны ажлын мөр + биет явц) ÷ нийт.
   * `id: 1` нь орон сууцны ажил гэж ХАСАГДАНА (давхардал), 2-р сард сүүлийн
   * биет явц (50₮) УРАГШ явна — эс бөгөөс муруй доош унана.
   *   01: (0 + 50)/1000 = 5   02: (100 + 50)/1000 = 15   03: (200 + 120)/1000 = 32
   */
  const ch = cashflowCurve(plan, 1000, 'month', undefined, new Map(), hm, new Set([1]));
  assert.deepEqual(ch.map((p) => p.pct), [5, 15, 32],
    'орон сууцны биет явц цэнхэр муруйд буруу нэгтгэгдэв');
  /* skipIds-гүй бол давхардана — 01: 15, 02: 25, 03: 42 (тест нь зөвхөн ялгааг батална) */
  const chDup = cashflowCurve(plan, 1000, 'month', undefined, new Map(), hm);
  assert.deepEqual(chDup.map((p) => p.pct), [15, 25, 42]);
  assert.ok(!('housePct' in ch[0]), 'ягаан муруйн талбар ХАСАГДСАН байх ёстой');
