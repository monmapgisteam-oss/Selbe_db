 ...
 * одоогийн байдлыг харна. Бичилт (`syncNegtgel`) нь ард нь, чимээгүй явна.
...
async function loadNegtgelFullRaw(): Promise<NegtgelFull> {
  ...
  try {
    ...
    void A.syncNegtgel(stored, calcRows);
  } catch (e) {
  ...
  return { rows, calc, live: calcRows != null };
}
...
export function negtgelProjectPct(
  ...
  for (const r of rows) {
    ...
    if (!c || c.inProject == null || c.actPct == null) continue;
    w += c.inProject;
