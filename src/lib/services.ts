...
export const HABEA = {
   ...
   * хүсэлт). Хоёулаа бүтцээрээ БАРАГ ижил: ~50 шалгах зүйл (`s01…s29`,
   * `j01…j04`, `h01…h06`, `k01…k13`, `e01…e07`) тус бүр 5 утгатай домэйнтэй,
   * дээр нь тэдгээрийн НИЙЛБЭР болох `cnt_*` таван талбар.
   * ⚠️ ЯЛГАА (2026-09-06, амьд метадата): `guitsetgegch`-д `site_block` ба
   *    `company_other` талбар БАЙХГҮЙ. `outFields: ['*']`-аар татдаг тул
   *    400 гарахгүй, тэдгээр нь `undefined` → «—» болно.
   * ⚠️ `site` · `company` · `shift` · `week` нь codedValue ДОМЭЙНТЭЙ —
   *    атрибутад КОД («b1», «mcc2», «day») ирнэ; нэрийг `habeaUzleg` нь
   *    үйлчилгээний метадатагаас хөрвүүлнэ.
   *
   * ⚠️ ХОЁУЛАА ХООСОН (0 мөр, 2026-09-06-нд амьдаар шалгав). Самбар нь
   * «Бүртгэл алга» гэж гарна — энэ нь алдаа БИШ. Өгөгдөл ирэхэд өөрөө
   * дүүрнэ; хоосон байгаа тул ямар нэг тоо ЗОХИОЖ бөглөж БОЛОХГҮЙ.
   *
   * ⚠️ НЭР нь үйлчилгээний ДОТООД нэртэй ЗӨРНӨ: `v11`-ийн давхарга нь
   * `hse_contractor_inspection_cn`, `guitsetgegch`-ийнх нь
   * `hse_site_inspection_v11` гэж нэрлэгдсэн. Энд ХЭРЭГЛЭГЧИЙН ӨГСӨН
   * нэршлийг барив (2026-09-06-нд тодруулж баталгаажуулсан) — дотоод нэрээр
   * «зөв болгох» гэж БҮҮ сольж хая.
   */
  uzleg: {
    v11: {
      title: tr('Ажлын байрны үзлэг V11'),
...
const catalogIdsOf = (key: GroupKey): string[] =>
  [...new Set([...GROUP_LAYERS[key], ...(CATALOG_EXTRA[key] ?? [])])]
    .filter((id) => LAYER_BY_ID[id] && !CATALOG_DUP_HIDDEN.has(id));
