...
/* BUS_cashflow (багцын төсөв/эх үүсвэр/сарын олголт) 2026-08-13-нд бүрмөсөн
   хасагдав — санхүүгийн бодит эх нь CASHFLOW_NEW (гэрээ) + IPC_LOG (олгосон акт). */

/* ══════════════════════ Санхүүжилт (шинэ харагдац) ══════════════════════ */

/**
 * IPC — Гүйцэтгэлийн ТӨЛБӨРИЙН АКТЫН лог (`ipc_0813/172`, 59 мөр, геометргүй).
 *
 * ⚠️ 2026-08-31-нд `IPC_/107` (90 мөр, IPC01…IPC44)-ийг ОРЛУУЛСАН. Ялгаа нь
 * зөвхөн багана биш, ГРЕЙН нь өөр:
 *   · Хог мөр АЛГА. Хуучин 90 мөрийн 31 нь акт БИШ байсан («Contract Price»
 *     4 псевдо-мөр, дугааргүй/дүнгүй мөрүүд) тул хэрэглэгч бүр
 *     `/^(IPC|APC|АРС)[-\s]?\d+/` шүүлт хийх ёстой байв. Одоо 59 мөр БҮГД
 *     жинхэнэ акт — ШҮҮЛТ ХЭРЭГГҮЙ, шүүвэл эсрэгээрээ буруу болно.
 *   · Гэрээний үнэ энд БАЙХГҮЙ — `CASHFLOW_NEW.fields.budget/contractAmount`-д.
 *     Хуучин хүснэгтэд актын дүнтэй нэг баганад холилдож нийлбэрийг 5.4 дахин
 *     хөөрөгдөж байсан.
 *   · Гэрээтэй холбоос `geree` (G01…G76) — багцаар тааруулах хэрэггүй болсон.
 *     8 актад гэрээ тодорхойгүй тул хоосон (олон гэрээт байгууллага).
 *   · Бодогддог багана (олгох дүн, үлдэгдэл, хувь, хоногийн тоо) ХАСАГДСАН —
 *     `ipcNet`, `ipcPaid`, `ipcDue`-ээр бод. Хадгалсан бол засвар бүрд хуучирна.
 *   · Огноо нь `esriFieldTypeDateOnly` — «2026-08-10» гэсэн МӨР болж ирнэ,
 *     epoch БИШ. `new Date(v)` биш, шууд харьцуулж/задал.
 */
export const IPC_LOG = {
  url: `${HJ}/ipc_0813/FeatureServer/172`,
  oid: "OBJECTID",
  /** `kind` (IPC06)-ийн утгууд */
  kinds: { advance: "УРЬДЧИЛГАА", work: "ГҮЙЦЭТГЭЛ" },
  /** `status` (IPC08)-ийн утгууд */
  statuses: { approved: "БАТЛАГДСАН", review: "ХЯНАГДАЖ БАЙНА" },
  fields: {
    id: "IPC01",             // Актын код (I01…)
    geree: "IPC02",          // ⚠️ Гэрээний код (G01…G76) — ХАРАХ утга. Хуучин
                             //    cashflow-ийн сарын мөрийг холбодог байсан; шинэ
                             //    хүснэгтэд тэр код БАЙХГҮЙ тул НЭГТГЭЛД ХЭРЭГЛЭХГҮЙ,
                             //    актыг БАГЦААР нэгтгэнэ (`pkg2` → `pkg`).
    pkg: "IPC03",            // Багц (үндсэн)
    pkg2: "IPC04",           // Дэд багц
    contractor: "IPC05",     // ⚠️ Гүйцэтгэгч — CASHFLOW_NEW-тэй ХАШИЛТААР зөрдөг
                             //    («"ББСМО" ХХК» ↔ «ББСМО ХХК») тул нэрээр тулгаж БОЛОХГҮЙ
    kind: "IPC06",           // УРЬДЧИЛГАА | ГҮЙЦЭТГЭЛ
    no: "IPC07",             // Дугаар (1,2,3…) — дэлгэцийн код нь `ipcCode()`
    status: "IPC08",         // Актын төлөв
    periodFrom: "IPC09",     // Хамрах хугацаа эхлэх
    periodTo: "IPC10",       // Хамрах хугацаа дуусах
    requestNo: "IPC11",      // Хүсэлтийн бичгийн дугаар
    submitDate: "IPC12",     // Хүсэлтийн бичгийн огноо
    quantityAmount: "IPC13", // Тоо хэмжээний дүн
    m812Date: "IPC14",       // М-8/12 огноо
    m812Amount: "IPC15",     // М-8/12 дүн
    approvedDate: "IPC16",   // М-7 огноо
    approvedAmount: "IPC17", // М-7 дүн (албан ёсны олгох)
    gross: "IPC18",          // Гүйцэтгэлийн дүн (актын нийт)
    clientDeduct: "IPC19",   // Захиалагчийн хяналтын суутгал
    advanceRecovery: "IPC20",// Урьдчилгаа эргэн төлөлт
    retention: "IPC21",      // Барьцаа
    authorDeduct: "IPC22",   // Зохиогчийн хяналт
    hmhNo: "IPC23",          // ХМХ бичгийн дугаар
    hmhDate: "IPC24",        // ХМХ бичгийн огноо
    dueDate: "IPC25",        // Төлөх ёстой огноо
    nhogDate: "IPC26",       // НХОГ огноо
    nhogAmount: "IPC27",     // НХОГ дүн
    payDate: "IPC28",        // Гүйлгээ 1 огноо
    paid: "IPC29",           // Гүйлгээ 1 дүн
    payDate2: "IPC30",
    paid2: "IPC31",
    payDate3: "IPC32",
    paid3: "IPC33",
  },
  /** Суутгалууд — олгох дүн = `gross` − эдгээрийн нийлбэр */
  deductions: ["IPC19", "IPC20", "IPC21", "IPC22"],
  /** Гүйлгээний дүнгүүд — үлдэгдэл = олгох дүн − эдгээрийн нийлбэр */
  payments: ["IPC29", "IPC31", "IPC33"],
} as const;
...
/** Актын ДЭЛГЭЦИЙН код: «APC-01» (урьдчилгаа) · «IPC-03» (гүйцэтгэл). */
export const ipcCode = (r: Row): string =>
  `${r[IPC_LOG.fields.kind] === IPC_LOG.kinds.advance ? "APC" : "IPC"}-${String(
    r[IPC_LOG.fields.no] ?? "",
  ).padStart(2, "0")}`;

/**
 * Актын СӨРӨГ цэвэр олголтыг НЭГ УДАА логддог хамгаалалт.
 * ⚠️ Дуудалт бүрд бичвэл 59 акт × render бүрд консол дүүрнэ — актын кодоор
 *    цээжилж НЭГ мөр л үлдээнэ.
 */
const ipcNegWarned = new Set<string>();

/**
 * Гүйцэтгэгчид ОЛГОХ дүн = гүйцэтгэлийн дүн − 4 суутгал.
 * `null` = гүйцэтгэлийн дүн (IPC18) ХЭМЖИГДЭЭГҮЙ — 0 БИШ.
 *
 * ⚠️ Хуучин `IPC40` (net) баганыг ХАСАВ — засвар бүрд хуучирдаг байсан.
 * Гүйлгээг ЭНД хасахгүй (тэр нь үлдэгдэл — `ipcDue`).
 *
 * ⚠️ 2026-09-04: буцах төрөл `number` → `number | null` БОЛОВ. Өмнө нь
 * `nz(gross)` нь ХООСОН гүйцэтгэлийн дүнг 0 болгодог байсан тул зөвхөн
 * суутгалтай акт СӨРӨГ олголт болж, нийлбэрийг чимээгүй ДООШ татдаг байв.
 * Бодит алдаа (амьд өгөгдөл, 2026-09-04): акт I30 — Багц 4-1, БАТЛАГДСАН,
 * IPC18 = хоосон, IPC20 (урьдчилгаа эргэн төлөлт) = 2,072,616,655.12 →
 * `ipcNet` = −2,072,616,655 ₮. Энэ нь `givenTotal` ба 2026-06-ны цэгт нэмэгдэж
 * хуримтлалын муруйг доош татаж байлаа (Багц 4-1: харагдах 68,121,667,446 ₮ ↔
 * жинхэнэ 70,194,284,101 ₮ — −2.95%). IPC18 нь 59 актын 38-д ХООСОН.
 * Одоо `finCard.netOrNull`-тэй ЯГ НЭГ дүрэм: `gross` хоосон/тоо биш бол `null`.
 *
 * ⚠️ ДУУДАГЧ ТАЛД: `null`-ыг АЛГАСАХ ёстой («хэмжигдээгүй»), `?? 0` гэж БҮҮ
 * дар — тэгвэл яг энэ алдаа буцаж гарна. Нийлбэрт бүх мөр хоосон бол үр дүн нь
 * `0` биш `null` (хэмжилтгүй) байх ёстой — `finCard.netTotalOrNull`-ыг үз.
 */
export const ipcNet = (r: Row): number | null => {
  const g = r[IPC_LOG.fields.gross];
  if (g == null || g === "") return null;
  const gross = Number(g);
  if (!Number.isFinite(gross)) return null;
  const net = gross - IPC_LOG.deductions.reduce((a, f) => a + nz(r[f]), 0);
  /* ⚠️ Сөрөг олголт нь ӨГӨГДЛИЙН АЛДАА (суутгал нь актын дүнгээс их) — нуухгүй,
     ил гаргана. Тооцооллыг зогсоохгүй: утга нь хэвээр буцна, зөвхөн лог. */
  if (net < 0) {
    const code = String(r[IPC_LOG.fields.id] ?? ipcCode(r));
    if (!ipcNegWarned.has(code)) {
      ipcNegWarned.add(code);
      console.warn(
        `[selbe] ipcNet: «${code}» актын цэвэр олголт СӨРӨГ (${net.toFixed(2)} ₮) — ` +
        `суутгал (IPC19–IPC22) нь гүйцэтгэлийн дүн (IPC18 = ${gross}) -ээс их. Өгөгдлийн алдаа.`,
      );
    }
  }
  return net;
...
/** Бодитоор шилжүүлсэн нийт дүн (3 гүйлгээ). */
export const ipcPaid = (r: Row): number =>
  IPC_LOG.payments.reduce((a, f) => a + nz(r[f]), 0);

/**
 * Үлдэгдэл — олгох ёстой мөртлөө хараахан шилжүүлээгүй дүн.
 * ⚠️ 2026-09-04: `ipcNet`-ийн дагуу `number | null` болов. Олгох дүн
 * хэмжигдээгүй бол ҮЛДЭГДЭЛ Ч хэмжигдэхгүй — гүйлгээг дангаар нь «сөрөг
 * үлдэгдэл» болгож харуулбал өмнөх алдаа өөр нэрээр давтагдана.
 */
export const ipcDue = (r: Row): number | null => {
  const net = ipcNet(r);
  return net == null ? null : net - ipcPaid(r);
};
