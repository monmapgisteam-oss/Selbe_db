...
/**
 * ТӨСЛИЙН НЭГТГЭЛ ГҮЙЦЭТГЭЛ — `Negtgel_guitsetgel/0` (352 мөр).
 *
 * ⚠️ 2026-09-25: ЭХ СОЛИГДОВ (хэрэглэгчийн заавар: «энэ service нэгтгэл
 * гүйцэтгэлийг солиё»). Хуучин `Tusul_guitsetgel/0` нь зөвхөн модны БҮТЭЦТЭЙ,
 * тоон 5 талбар нь 0/197 ХООСОН байсан тул порталд утгыг `Cashflow_0909`-ийн
 * шатны талбараас хөтөч дотор БОДДОГ байв. Шинэ үйлчилгээнд утгууд БӨГЛӨГДСӨН
 * тул порталд одоо зөвхөн УНШИНА, бодохгүй.
 *
 * ⚠️ УТГУУД 0–1 БУТАРХАЙ (жишээ `TOSOLD_EZLEH_HUVI = 0.05`) — `pct()` нь
 * 100-аар үржүүлдэггүй тул уншихдаа ×100 (`negtgel.ts`). Эс бөгөөс 5% нь
 * «0.1%» болно.
 *
 * ⚠️ `DD` КОД ДАВХАРДСАН: «3» нь «Газар чөлөөлөлт» ба «Зөвшөөрөл», «5.2.3»
 * нь дөрвөн мөрд. Модыг кодоор биш, БАЙРЛАЛ + ГҮНЭЭР угсарна.
 *
 * ⚠️ Төлөвлөгөө ба биелэлт нь ГУРВАН хувилбартай (гэрээ · гэрээ ГЧ ·
 * гүйцэтгэгч) — эх хүснэгтийн бүтцийг хадгалж гурвууланг нь харуулна.
 */
export const TUSUL_NEGTGEL = {
  url: `${HJ}/Negtgel_guitsetgel/FeatureServer/0`,
  oid: "OBJECTID",
  fields: {
    /** Ажлын задаргааны код — «1.2.1.1» */
    code: "DD",
    /** Багцын дугаар — «Багц-1» (навч мөрд) */
    bagts: "BAGTS_DUGAAR",
    name: "AJLYN_NER",
    /** Урьдчилсан төсөвт өртөг, ₮ */
    budget: "URID_TOSOVT_ORTOG",
    /** Эцэг хэсэгтээ эзлэх жин (0–1) */
    inSection: "HESEGT_EZLEH",
    /** Төсөлд бүхэлд нь эзлэх жин (0–1) */
    inProject: "TOSOLD_EZLEH_HUVI",
    /** Төлөвлөгөөт хувь (0–1) — гэрээгээр · гэрээ ГЧ · гүйцэтгэгчийн */
    planGeree: "TOLOV_HUVI_GEREE",
    planGch: "TOLOV_HUVI_GEREE_GCH",
    planGuits: "TOLOV_HUVI_GUITSETGEGCH",
    /** Гүйцэтгэлийн хувь (0–1) */
    actPct: "GUITSETGELIIN_HUVI",
    /** Төлөвлөгөөний биелэлт (0–1) — гурван хувилбарт харгалзана */
    perfGeree: "BIELELT_GEREE",
    perfGch: "BIELELT_GEREE_GCH",
    perfGuits: "BIELELT_GUITSETGEGCH",
  },
} as const;
...
/**
 * 3D МЕШИЙН ХУВИЛБАР — «Нислэг 2» (2026-09-17, төслийн талбайн БҮТЭН меш)
 * ба «Нислэг 1» (Сэлбэ 1, 2). Сонгогч нь зургийн доод голд тусдаа хоёр товч.
 *
 * ⚠️ 2026-09-25 (хэрэглэгчийн шийдвэр): шинэ меш урьд нь ЗӨВХӨН «Меш
 *    харьцуулах» товчинд түр гардаг байв. Одоо 3D-ийн байнгын хувилбар бөгөөд
 *    АНХДАГЧ нь шинэ. Хуучин руу `MapCanvas`-ийн сонгогчоор шилжинэ.
 * ⚠️ `SCENE` (хуучин) ХЭВЭЭР — `SuitMap.tsx` өөрийн гогцоонд ашигладаг.
 * ⚠️ Харьцуулалт («Меш харьцуулах») нь НӨГӨӨ хувилбарыг `MESH_CMP_PREFIX`
 *    id-тай нэмдэг. `scene:` угтвар БИШ: тэр нь горим солиход бүрмөсөн хасдаг
 *    циклд ордог тул харьцуулалтын давхарга түүнээс гадуур байх ёстой.
 * ⚠️ Шинэ мешийн хамрах талбай хуучнаас ИЛҮҮ ХОЙШ (ymax 6103061 ↔ 6102273) —
 *    харьцуулахад хойд захад зөвхөн шинэ харагдах нь хэвийн.
 * ⚠️ Шинэ меш тусдаа env/fallback-гүй — бусад меш (`Selbe1_slpk` г.м.)-тэй нэг
 *    сервер дээрх `Hosted` дэд үйлчилгээ.
 */
export type MeshVer = 'new' | 'old';

export const MESH_VERSIONS = {
  new: {
    title: tr('Нислэг 2 (2026-09-17)'),
    layers: [
      {
        key: "mesh0917",
        title: tr('Бодит загвар — нислэг 2 (2026-09-17)'),
        url: `${UBHUB_SCENE}/Selbe_mesh_0917/SceneServer`,
      },
    ],
  },
  old: {
    title: tr('Нислэг 1 (Сэлбэ 1, 2)'),
    layers: SCENE.layers,
  },
} as const;

export const DEFAULT_MESH_VER: MeshVer = 'new';

/** Харьцуулалтын меш давхаргын id угтвар (`mesh:cmp:<key>`) */
export const MESH_CMP_PREFIX = 'mesh:cmp:';
...
export const GROUP_LAYERS: Record<GroupKey, string[]> = {
  ...
  /* ⚠️ 2026-09-21: `sb:1` (`data`/35) нь `nogoon` (`data`/118)-той ИЖИЛ URL
     БИШ — нэгтгэсэн ганц полигон (587,555 м², бүсгүй) ↔ 807 объект (545,102 м²,
     `RefName_12`). Энд `sb:1` үлдсэн нь САНААТАЙ: `PLAN_LAYER_IDS`-ийн нийлбэр
     бүсээс ГАДУУРХ ногооныг ч агуулна. Каталогийн мөр ба `Dashboard`-ын
     `CATALOG_IDS` нь `nogoon`-оос тоолно (`CATALOG_DUP_HIDDEN`-ийн тайлбар). */
  green: ["sb:1", "sb:0"],
 ...
 *  · `sb:1`↔`nogoon` — ⚠️ 2026-09-21: ОДОО ИЖИЛ URL БИШ, харин НЭГ ӨГӨГДЛИЙН
 *    ХОЁР ХЭЛБЭР. Амьд шалгалт: `sb:1` = `data`/35 (`Ногоон_байгууламж_0813`,
 *    807 хэсгийг НЭГТГЭСЭН ГАНЦ полигон, 1 мөр, 587,555 м², бүсийн талбаргүй);
 *    `nogoon` = `data`/118 (`nogoon_baiguulamj_analysis`, 807 объект,
 *    545,102 м², `RefName_12`-оор 55 бүсэд тайрагдсан). Каталогт `nogoon`
 *    ҮЛДЭНЭ (объект тус бүр, бүсээр шүүгддэг), `sb:1` нуугдана — хоёулаа
 *    гарвал «Ногоон байгууламж» нэг нэрээр 1 ш ба 807 ш гэсэн хоёр мөр болно.
 *    ХААНА АЛЬ НЬ: `GROUP_LAYERS.green` → `PLAN_LAYER_IDS` (төслийн НИЙЛБЭР,
 *    `ViewPanel`) нь `sb:1`-ээс (бүсээс ГАДУУРХ ногоон орсон бүтэн дүн);
 *    каталогийн мөр, `Dashboard`-ын `CATALOG_IDS`, `INITIAL_MAP_LAYERS`,
 *    анализ (`land.ts`, `ersdel.ts`) нь `nogoon`-оос. Нэг дэлгэц дээр хоёр
 *    эхийн тоог зэрэгцүүлж БҮҮ харуул — 4.2 га зөрнө.
...
const CATALOG_DUP_HIDDEN: ReadonlySet<string> = new Set([
  "sb:4", "sb:2", "sb:3", "sb:15",
  /* ⚠️ 2026-09-21: `sb:1` нь `nogoon`-той URL-аар БИШ, ӨГӨГДЛӨӨР давхардана
     (нэгтгэсэн 1 полигон ↔ 807 объект) — дээрх тайлбарыг үз. */
  "sb:1",
