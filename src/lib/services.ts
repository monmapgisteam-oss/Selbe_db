...
/** Сарын тэнхлэгийн нэг цэг — CF баганатай (төлөвлөгөөт 12 сар) эсвэл багангүй (сунгалт). */
export type CfMonth = {
  label: string;
  amount: string | null;
...
/**
 * САРЫН ТЭНХЛЭГ — CF хүснэгтийн 12 сар + ӨНӨӨДРИЙГ хүртэлх сунгалт.
 *
 * ⚠️ ЯАГААД (2026-08-29). CF хүснэгтийн сарын баганууд 2026-09-өөр төгсдөг.
 * Тэнхлэгийг шууд `CASHFLOW2.months`-оос авбал 2026-10-01-ээс хойш:
 *   · биет гүйцэтгэлийн шинэ хэмжилт орох САРГҮЙ тул график ЦАРЦАНА,
 *   · IPC-ийн шинэ олголт бүгд 2026-09-ийн нүдэнд овоорно.
 * Алдаа гарахгүй — зөвхөн тоо ЧИМЭЭГҮЙ хуучирна, хамгийн муу төрлийн эвдрэл.
 *
 * Сунгалтын сард CF багана БАЙХГҮЙ тул талбарын кодууд нь `null` —
 * төлөвлөгөө тэнд жинхэнээсээ алга, хэрэглэгч 0 биш «төлөвлөгөөгүй» гэж
 * уншина. Бодит (олгосон, биет) утгууд харин жинхэнэ сардаа орно.
 *
...
export function cfMonthAxis(now: Date = new Date()): CfMonth[] {
  const out: CfMonth[] = CASHFLOW2.months.map((m) => ({ ...m }));
  const nowYm = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
  let [y, mo] = out[out.length - 1].label.split("-").map(Number);
  for (let guard = 0; guard < 36 && out[out.length - 1].label < nowYm; guard += 1) {
    mo += 1;
    if (mo > 12) { mo = 1; y += 1; }
    out.push({
      label: `${y}-${String(mo).padStart(2, "0")}`,
      amount: null, amountCum: null, pct: null, pctCum: null,
    });
  }
  return out;
}
