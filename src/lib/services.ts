 ...
 *   · Гэрээний үнэ энд БАЙХГҮЙ — `CASHFLOW2.fields.budget/contractAmount`-д.
...
export const IPC_LOG = {
  ...
  fields: {
    ...
    geree: "IPC02",          // Гэрээний код → CASHFLOW2.fields.geree
    pkg: "IPC03",            // Багц (үндсэн)
    pkg2: "IPC04",           // Дэд багц
    contractor: "IPC05",     // Гүйцэтгэгч — CASHFLOW2-той ҮСЭГ БҮРЭЭР тэнцүү
...
/**
 * CASHFLOW2 — Гэрээ бүрийн САНХҮҮЖИЛТИЙН ХУВААРЬ (`cashflow_0813/173`, 209 мөр).
 *
 * ⚠️ 2026-08-31-нд `Cashflow/106` (76 мөр, CF001…CF255)-ийг ОРЛУУЛСАН.
 * ЗАГВАР НЬ ЭРС ӨӨР — «нэг гэрээ = нэг мөр, 12 сар нь БАГАНА» байсныг
 * «нэг мөр = гэрээний нэг ҮЕ» болгосон:
 *
 *   CF002 (`rowType`) = `ГЭРЭЭ` — гэрээ бүрд ЯГ НЭГ мастер мөр (76). Гэрээний
 *                       бүх шинж (нэр, төсөв, огноо, эх үүсвэр) ЗӨВХӨН энд.
 *               `САР` — тухайн сарын хэмжилт (131). Хэмжилтгүй сар мөр ҮҮСГЭХГҮЙ.
 *    `ӨМНӨХ ШИЛЖҮҮЛСЭН` — өмнөх онд шилжүүлсэн дүн (2).
 *
 * ⚠️ Тиймээс НИЙЛБЭР бодохдоо мөрийн төрлөөр ЗААВАЛ шүү (`where`-ийг ашигла):
 *   · гэрээний тоо/төсөв → `where.master`; эс тэгвээс нэг гэрээ 14 удаа тоологдоно
 *   · сарын урсгал → `where.month`
 * Багц/ангилал/гүйцэтгэгч (CF005–CF008) нь мөр БҮРД давтагдана — сарын дүнг
 * багцаар бүлэглэхэд НЭГ query хангалттай.
 *
 * ⚠️ Хуучин CF_MONTH_BASES-ийн ЖИГД БУС алхмын занга (2026-01 нь CF097-оос)
 * УСТСАН — сар одоо баганы код биш, `CF003`/`CF004` (жил/сар) утга.
 *
 * ⚠️ Бодогддог багана бүгд ХАСАГДСАН: хувь, өссөн дүн, суутгалын томьёо
 * (dun×0.05/0.02/0.01/0.02), төлөх дүн, урьдчилгааны үлдэгдэл, жилийн нийлбэр
 * мөр. Хэрэгтэй бол ХАРУУЛАХ үедээ бод — хадгалсан бол засвар бүрд зөрнө.
 *
...
export const CASHFLOW2 = {
  ...
  oid: "OBJECTID",
  /** `rowType` (CF002)-ийн утгууд */
  rows: { master: "ГЭРЭЭ", month: "САР", prev: "ӨМНӨХ ШИЛЖҮҮЛСЭН" },
  /** Бэлэн WHERE — мөрийн төрөл ЗААВАЛ шүүнэ, эс тэгвээс давхар тоолно */
  where: {
    master: "CF002 = 'ГЭРЭЭ'",
    month: "CF002 = 'САР'",
    /** Хэмжилттэй бүх мөр (сар + өмнөх шилжүүлсэн) */
    period: "CF002 <> 'ГЭРЭЭ'",
  },
  fields: {
    geree: "CF001",          // Гэрээний код G01…G76 — ipc-тэй холбогдох түлхүүр
    rowType: "CF002",        // ГЭРЭЭ | САР | ӨМНӨХ ШИЛЖҮҮЛСЭН
    year: "CF003",           // Жил (үеийн мөрөнд)
    monthNo: "CF004",        // Сар 1–12 (зөвхөн САР мөрөнд)
    type: "CF005",           // Төслийн ангилал (ТЭЗҮ, инженер…)
    pkg: "CF006",            // Багц — үндсэн («БАГЦ-6»)
    pkg2: "CF007",           // Дэд багц («БАГЦ-6.2»); дэдгүй бол pkg-тай тэнцүү
    contractor: "CF008",     // Гүйцэтгэгч
    /* ── үеийн мөрийн хэмжилт ── */
    amount: "CF009",         // Тухайн үеийн мөнгөн дүн
    advance: "CF014",        // Урьдчилгаа ОЛГОЛТ (тухайн сард)
    advanceRepay: "CF015",   // Урьдчилгаа эргэн төлөлт
    opened: "CF016",         // Эрх нээсэн дүн
    /* ── мастер (ЗӨВХӨН ГЭРЭЭ мөрөнд) ── */
    name: "CF017",           // Ажлын нэр
    budget: "CF018",         // Төсөвт өртөг
    amountNote: "CF019",     // Дүнгийн тайлбар
    startDate: "CF020",      // Эхлэх огноо
    endDate: "CF021",        // Дуусах огноо
    orderDate: "CF022",      // Захирамжийн огноо
    orderNo: "CF023",        // Захирамжийн дугаар
    orderTotal: "CF024",     // Захирамжийн нийт дүн
    client: "CF029",         // Захиалагчийн хяналтын байгууллага
    contractDate: "CF030",   // Гэрээний огноо
    contractNo: "CF031",     // Гэрээний дугаар
    contractNote: "CF032",   // Гэрээний тайлбар
    contractAmount: "CF033", // Гэрээний эрхийн дүн
    advanceGuarantee: "CF034", // Урьдчилгааны баталгааны дүн
    advanceDeduct: "CF035",  // Урьдчилгааны хяналтын суутгал
    extraAmount: "CF036",    // Нэмэлт дүн
  },
  /**
   * Санхүүжилтийн эх үүсвэр — `total` нь ГЭРЭЭ мөрийн нийт дүн, `period` нь
   * тухайн үеийн задаргаа. Хоёулаа ижил дараалалтай.
   */
  sources: [
    { total: "CF025", period: "CF010", label: tr('Үнэт цаас'), color: "#3387b8" },
    { total: "CF026", period: "CF011", label: tr('Төслийн орлого'), color: "#22c55e" },
    { total: "CF027", period: "CF012", label: tr('Нийслэлийн төсөв'), color: "#f59e0b" },
    { total: "CF028", period: "CF013", label: tr('НЗД нөөц'), color: "#a855f7" },
  ],
  /** Төлөвлөгөөт цонх — хуваарьт орсон хамгийн эрт/сүүлийн сар */
  planFrom: "2025-10",
  planTo: "2026-09",
} as const;

/** Үеийн мөрийн сарын шошго: «2026-08». САР бус мөрөнд `null`. */
export const cfMonthKey = (r: Row): string | null => {
  const y = Number(r[CASHFLOW2.fields.year]);
  const m = Number(r[CASHFLOW2.fields.monthNo]);
  if (!Number.isFinite(y) || !Number.isFinite(m) || !m) return null;
  return `${y}-${String(m).padStart(2, "0")}`;
};

/** Сарын тэнхлэгийн нэг цэг. */
export type CfMonth = {
  label: string;
  /** Төлөвлөгөөт цонхонд багтаж байгаа эсэх (сунгалтын сард `false`) */
  planned: boolean;
};

/**
 * САРЫН ТЭНХЛЭГ — төлөвлөгөөт цонх + ӨНӨӨДРИЙГ хүртэлх сунгалт.
 *
 * ⚠️ ЯАГААД (2026-08-29). Хуваарийн сүүлийн сар бол 2026-09. Тэнхлэгийг зөвхөн
 * хуваариас авбал 2026-10-01-ээс хойш: биет гүйцэтгэлийн шинэ хэмжилт орох
 * САРГҮЙ тул график ЦАРЦАНА, IPC-ийн шинэ олголт бүгд 2026-09-ийн нүдэнд
 * овоорно. Алдаа гарахгүй — тоо ЧИМЭЭГҮЙ хуучирна, хамгийн муу төрлийн эвдрэл.
 *
 * ⚠️ Хуваарьт ЦООРХОЙ бий (2026-01-д ямар ч хэмжилт алга) тул тэнхлэгийг
 * өгөгдөлд БАЙГАА саруудаас биш, эхлэл→төгсгөлийн ТАСРАЛТГҮЙ мужаас угсарна —
 * эс тэгвээс график нэг сар алгасаад хуурамч тасалдал үзүүлнэ.
 *
 * ⚠️ Сунгалтыг 36 сараар хязгаарлана — цаг буруу тохируулсан машин дээр
 * тэнхлэг хязгааргүй уртсахаас сэргийлнэ.
 */
export function cfMonthAxis(now: Date = new Date()): CfMonth[] {
  const out: CfMonth[] = [];
  const [fy, fm] = CASHFLOW2.planFrom.split("-").map(Number);
  const nowYm = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
  let y = fy;
  let mo = fm;
  for (let guard = 0; guard < 12 + 36; guard += 1) {
    const label = `${y}-${String(mo).padStart(2, "0")}`;
    out.push({ label, planned: label <= CASHFLOW2.planTo });
    if (label >= CASHFLOW2.planTo && label >= nowYm) break;
    mo += 1;
    if (mo > 12) { mo = 1; y += 1; }
  }
  return out;
