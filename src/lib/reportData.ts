 ...
 *
 * ⚠️ ЭНЭ ХҮСНЭГТЭД «бодитоор олгосон» дүн БАЙХГҮЙ — тэр нь ХӨРӨНГӨ
 * ОРУУЛАЛТЫН ГҮЙЦЭТГЭЛИЙН бүртгэл (`HO_IPC` —
 * `HO_guitsetgel_arcgis_csv/196`, 45 мөр). Finance/ExecKpi/PkgFin бүгд
 * тэндээс уншдаг, тайлан ч мөн адил.
 *
 * ⚠️ 2026-09-09: `IPC_LOG` (`ipc_0813/172`, 59 мөр) БҮРМӨСӨН ХАЯГДСАН —
 * хэрэглэгч түүнийг ТЕСТ ӨГӨГДӨЛ гэж тогтоов («хуучин үйлчилгээ дээр
 * байсан зүйлс одоо огт хамаагүй»). Хоёр эхийн тоог ХАРЬЦУУЛАХГҮЙ.
 * Хамт хасагдсан ойлголтууд: СУУТГАЛ (барьцаа, урьдчилгааны эргэн
 * төлөлт, зохиогчийн/захиалагчийн хяналт), «цэвэр дүн» (`ipcNet`),
 * «төлөгдөөгүй үлдэгдэл» (`ipcDue`), актын ТӨЛӨВ («хянагдаж байна»),
 * «төлөх ёстой огноо». Шинэ эхэд мөр БҮР нь АЛЬ ХЭДИЙН хийгдсэн
 * гүйлгээ бөгөөд `dun` нь БОДИТ ОЛГОСОН дүн.
 *
 * ⚠️ ШИНЭ ЭХИЙН ГРЕЙН: мөр = НЭГ ТӨЛБӨРИЙН ГҮЙЛГЭЭ. Гэрээний талбар
 * (`tosov_niit`, `gereet_tosov_niit`…) нь `geree_kod` бүрд ДАВТАГДАНА
 * — 45 мөр = 22 гэрээ. Энд бид ЗӨВХӨН `dun`-ыг (төлбөрийн түвшний
 * талбар) уншдаг тул давхардлын эрсдэлгүй; гэрээний түвшний тоо
 * хэрэгтэй болбол `@/lib/ipc`-ийн `hoTotals()`-оос ав, ЭНД БОДОХГҮЙ.
 *
 ...
 *
 * ⚠️ 2026-09-09: `HO_IPC` нь ДӨРӨВ ДЭХ ЗЭРЭГЦЭЭ хос тоо авчирсан
 *   (амьд хэмжилт): төсөвт өртөг 2,090.20 тэрбум (`tosov_niit`) ба
 *   гэрээт төсөв 2,005.71 тэрбум (`gereet_tosov_niit`), хэмнэлт 84.49.
 *   Эдгээр нь CASHFLOW_NEW-ийн 2,512.6 / 2,073.1-ээс ЗӨРНӨ — учир нь
 *   HO нь ЗӨВХӨН 22 гэрээг (CF-д 76) хамардаг. ХОЁУЛАА ЗӨВ, өөр өөр
 *   хамрах хүрээтэй. Энэ файл нь ГЭРЭЭНИЙ тоог CASHFLOW_NEW-ээс,
 *   ОЛГОСОН дүнг HO_IPC-ээс авдаг — хольж БҮҮ уншаарай.
 *
...
export type ReportExtra = {
  ...
  finance: {
     ...
     *
     * ⚠️ 2026-09-09: СЕМАНТИК ӨӨРЧЛӨГДСӨН. Урьд нь `ipcNet()` (актын
     *    гүйцэтгэлийн дүн − 4 суутгал) байв; шинэ эхэд суутгал ОГТ
     *    БАЙХГҮЙ бөгөөд `dun` нь бодит олгосон дүн. Мөн энэ нь
     *    УРЬДЧИЛГАА ТӨЛБӨРИЙГ Ч БАГТААНА (хэрэглэгчийн шийдвэр:
     *    «олгосон» = гүйцэтгэгч компанид өгсөн БҮХ санхүүжилт).
     *    Амьдаар 530.87 тэрбум ₮ = урьдчилгаа 314.01 + гүйцэтгэл 216.86.
     *    Хуучин тоо 318.42 тэрбум байсан — өмнөх дэлгэцийн тоог санаж
...
/*
 * ⚠️ 2026-09-09: хоёр үеийн өмнөх `isRealAct` шүүлтийн түүхэн тайлбар
 * (хуучин `IPC_/107`-гийн 90 мөрөөс 31 хог мөр хасах) БҮРМӨСӨН ХАСАГДАВ
 * — тэр нь хоёр удаа орлогдсон эх сурвалжид хамаарах бөгөөд шинэ
 * `HO_IPC`-д огт хамааралгүй. Шинэ эхийн 45 мөр БҮГД жинхэнэ бүртгэгдсэн
 * ТӨЛБӨР — ШҮҮЛТ ХЭРЭГГҮЙ.
 */

export const loadFinance = cached(loadFinanceRaw, 5 * 60_000, ['CASHFLOW_NEW', 'HO_IPC']);

async function loadFinanceRaw(): Promise<ReportExtra['finance']> {
  const F = CASHFLOW_NEW.fields;
  const [rows, ho] = await Promise.all([
    ...
    /* ⚠️ 2026-09-10 (tailan merge): `Cashflow_0909`-т сарын задаргааны 681 мөр
       ДООШ нэмэгдсэн тул `CF_WORK_WHERE` ЗААВАЛ — эс бөгөөс 78 гэрээ 759 болж
       нийт төсөв, багцын тоо бүгд эвдэрнэ. */
    queryFeatures(CASHFLOW_NEW.url, { where: CF_WORK_WHERE, outFields: ['*'] }),
    /* ⚠️ ГАНЦ талбар хангалттай: шинэ эхэд СУУТГАЛ БАЙХГҮЙ тул `dun` нь
       аль хэдийн бодит олгосон дүн. Хуучин 5 багана (gross + 4 суутгал)
       татдаг байсан нь хэрэггүй болов.
  ...
  /*
   * «Бодитоор олгосон» — HO төлбөрийн бүртгэлийн `dun` нийлбэр.
   ...
   * худал тоо байв.
   * ⚠️ ШҮҮЛТГҮЙ — 45 мөр бүгд жинхэнэ бүртгэгдсэн төлбөр. Багцаар шүүх
   *    хуучин хамгаалалт хасагдсан хэвээр: багцаар шүүвэл багцад
   *    холбогдоогүй төлбөрүүд чимээгүй алдагдана (доор).
   *
   * ⚠️ «ОЛГОСОН» = БҮХ ТӨЛБӨР (урьдчилгаа + гүйцэтгэл), хэрэглэгчийн
   *    шийдвэрийн дагуу: «IPC бол багц ажлын гүйцэтгэгч компанид өгсөн
   *    ТӨЛБӨР… энэ бүхэлдээ гүйцэтгэгч компанид өгсөн санхүүжилт».
   *    `tulult_turul`-аар ШҮҮХГҮЙ — зөвхөн «Гүйцэтгэл»-ийг тоолвол
   *    314.01 тэрбум ₮ (урьдчилгаа) чимээгүй алдагдана.
   *
   * ⚠️ ДҮРЭМ — ТӨСЛИЙН НИЙТ ОЛГОЛТ = БҮХ МӨРИЙН `dun` НИЙЛБЭР. Багцаар
   *    задалсан аливаа Map-ыг нийлүүлж төслийн дүн ГАРГАЖ БОЛОХГҮЙ.
   *    ⚠️ ЗӨРҮҮНИЙ ШАЛТГААН 2026-09-09-нд ӨӨРЧЛӨГДСӨН: хуучинд «8 актын
   *    `IPC03`/`IPC04` хоёулаа NULL» байв; ШИНЭД нь ДИАПАЗОН МӨР —
   *    «Багц-1-4» (876,465,164 ₮) ба «БАГЦ-10,  БАГЦ-11, БАГЦ-13,
   *    БАГЦ-15» (5,094,952,269 ₮) хоёрыг `pkgKeyOf` нь `''` болгож
   *    хамгаалдаг (`bagtsKey('Багц-1-4')` = `БАГЦ14` = БОДИТ Багц 14-ийн
   *    түлхүүр тул хамгаалалтгүй бол буруу эзэнд наалдана).
   *    АМЬД ХЭМЖИЛТ (2026-09-09, HO_guitsetgel/196, 45 мөр):
   *      · жинхэнэ нийт  530,872,795,391 ₮ (530.87 тэрбум)
   *      · багцын нийлбэр 524,901,377,959 ₮ (524.90 тэрбум)
   *      · зөрүү         5,971,417,432 ₮ (2 диапазон мөр)
   *    Тайлан · PDF · схем ЭНЭ тоог хэрэглэдэг тул зөрүү нь ЭНД биш,
   *    багцын Map-ыг нийлүүлдэг талд (`Finance.givenTotal` → `ExecKpi`)
   *    байна. Энэ мөрийг «дашбоардтай тааруулъя» гэж багцаар ШҮҮВЭЛ
   *    бодит олголтыг 5.97 тэрбум ₮-ээр дутуу тайлагнана.
   */
  /*
   * ⚠️ `null ≠ 0` — ЭНД `?? 0` БИЧИХГҮЙ. `dun` нь 45-ийн 2 мөрд ХООСОН
   *    (БАГЦ-6.3 гэрээ бүхэлдээ төлбөргүй, ХО-0045 кодгүй гэрээ).
   *    «хэмжигдээгүй» ба «тэг олголт» хоёр ӨӨР: `null` мөрийг
   *    НИЙЛБЭРЭЭС АЛГАСНА (тоологдох мөрийн тоо багасна, дүн гуйвахгүй).
   * ⚠️ Эргэж гарах хувилбар: хэн нэг нь `a + (hoAmount(r) ?? 0)` гэж
   *    «хялбарчилвал» дүн ижил хэвээр ч санаа нь будлиантаж, дараагийн
   *    хүн 0-г бодит олголт гэж уншина. Энэ нь 2026-09-04-ний I30
...
export const loadReportExtra = cached(loadReportExtraRaw, 5 * 60_000,
  ['BAGTS_SHEET', 'CASHFLOW_NEW', 'HO_IPC', 'PARCEL_LEFT', 'HABEA', 'BAGTS_NEGTGEL']);
