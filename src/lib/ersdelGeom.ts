...
export type DamageRow = {
  ...
  /** Өртсөн объектын тоо */
  n: number;
  /** Талбай (м²) — зөвхөн полигон давхаргад */
  area: number;
  /** Урт (м) — зөвхөн шугаман давхаргад */
  length: number;
  /** Үнэлгээ (₮) */
  cost: number;
  /** Өртсөн объектын геометр — улаанаар зурахад */
  graphics: Graphic[];
  /** Хэт олон объект тул геометр татаагүй (зөвхөн тоо) */
  truncated: boolean;
};
  ...
  for (const id of layerIds) {
    ...
    try {
      ...
      rows.push({
        ...
        truncated: n > res.features.length,
      });
    } catch {
