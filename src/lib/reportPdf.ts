...
export function buildReportDoc(
  ...
  /* ⚠️ 2026-09-21: `rows`-ийг дамжуулна — хамгийн өндөр/бага багц ХҮСНЭГТИЙН
     дүрмээр (`reportData.bagtsExtremes`), дэлгэц (`Tailan.tsx`)-тэй нэг эх. Урьд нь
     PDF `progress.byBagts` (зөвхөн тайлантай блокийн дундаж) уншиж зөрдөг байв. */
  const d = buildFindings(extra, rows);
  ...
  return {
    ...
    content: [
      ...
      /* ⚠️ 2026-09-21: §1-ийн танилцуулга, шошго, тайлбар ДЭЛГЭЦТЭЙ (`Tailan.tsx` §1)
         ҮГ ҮГЭЭР ижил — «Төслийн нийт гүйцэтгэл» нэрээр порталд гурван өөр
         тодорхойлолт явдаг тул нэр бүр юу болохоо хэлнэ (тоо өөрчлөгдөөгүй). */
      ...
      /* ⚠️ 2026-09-06: «Сар бүрийн санхүүжилтийн хуваарь» (7.2) ХАСАГДСАН —
         сарын төлөвлөгөө нь хуучин `cashflow_0813`-ийнх байсан бөгөөд тэр
         үйлчилгээ хаягдсан. Дэлгэцийн Tailan.tsx-д мөн адил. */
      ...
      { table: { headerRows: 1, widths: ['*', 38, 105, 105], body: [
        /* ⚠️ 2026-09-21: «Гэрээлсэн» — зөвхөн CONTRACTED мөрийн дүн (дэлгэцтэй ижил) */
        [th(tr('Төрөл')), th(tr('Ажил'), true), th(tr('Төсөв'), true), th(tr('Гэрээлсэн'), true)],
